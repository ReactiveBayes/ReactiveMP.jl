# Tables for the five variants from micro_f.tsv, model_f.tsv and spec_f.tsv: for each case
...
for path, micro in (('micro_f.tsv', True), ('model_f.tsv', False)):
    d, order = load(path, micro)
    print(f"\n## {path}  (median of per-round minima; spread = max/min of the minima; allocs from round 1)\n")
    print("| case | " + " | ".join(VARS) + " | C/A | E/A | F/A | spread A,C | allocs A/C/E/F |")
    print("|---" * (len(VARS) + 6) + "|")
    for n in order:
        ...
        print(f"| {n} | " + " | ".join(fmt(med[v], micro) for v in VARS) +
              f" | {med['C']/a:.2f} | {med['E']/a:.2f} | {med['F']/a:.2f} | {spread['A']:.3f},{spread['C']:.3f} | {allocs} |")

try:
    rows = collections.defaultdict(list)
    for line in open('spec_f.tsv'):
        p = line.rstrip('\n').split('\t')
