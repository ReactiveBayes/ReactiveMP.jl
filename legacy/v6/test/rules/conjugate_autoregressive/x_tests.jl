...
@testitem "rules:ConjugateAR:x (delegates to AR)" tags = [:rules] begin
    using ReactiveMP,
    ...
    @testset "x message equals AR(:x) with effective (q_θ, q_γ)" begin
        ...
        for order in (1, 2)
            ...
            got = @call_rule ConjugateAR(:x, Marginalisation) (
                m_y = m_y, q_w = q_w, meta = meta,
            )
            exp = @call_rule AR(:x, Marginalisation) (
                m_y = m_y, q_θ = q_θ, q_γ = q_γ, meta = meta,
            )
    ...
    @testset "x message (q_y variant) equals AR(:x)" begin
        ...
        for order in (1, 2)
            ...
            got = @call_rule ConjugateAR(:x, Marginalisation) (
                q_y = q_y, q_w = q_w, meta = meta,
            )
            exp = @call_rule AR(:x, Marginalisation) (
                q_y = q_y, q_θ = q_θ, q_γ = q_γ, meta = meta,
            )
            @test same_normal(got, exp)
        end
    end
