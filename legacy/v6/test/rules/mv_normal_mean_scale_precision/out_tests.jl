...
@testitem "rules:MvNormalMeanPrecision:out" tags = [:rules, :alloc] begin
    using ReactiveMP, BayesBase, Random, ExponentialFamily, Distributions

    import ReactiveMP: @test_rules

    @testset "Variational: (q_out::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(
            :out, Marginalisation
        ) [
            (
                input = (
            ...
            (
                ...
                output = MvNormalMeanScalePrecision([2.0, 1.0], 6.0),
            ),
            (
                input = (
    ...
    @testset "Structured variational: (m_μ::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(
            ...
            (
                ...
                output = MvNormalMeanCovariance([2.0, 1.0], [3.5 2.0; 2.0 4.5]),
            ),
            (
                input = (
                    m_μ = MvNormalMeanCovariance(
    ...
    @testset "Structured variational: (m_μ::MvNormalMeanScalePrecision, q_γ::Gamma)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(
            ...
            (
                input = (
                    m_μ = MvNormalMeanScalePrecision([2.0, 1.0], 3.0),
                    q_γ = Gamma(1.0, 1.0),
                ),
                output = MvNormalMeanScalePrecision(
                    [2.0, 1.0], 3.0 * 1.0 / (3.0 + 1.0)
                ),
            ),
            (
                input = (
                    m_μ = MvNormalMeanScalePrecision([0.0, 0.0], 4.0),
            ...
            (
                input = (
                    m_μ = MvNormalMeanScalePrecision([3.0, -1.0], 2.0),
    ...
    @testset "Performance: MvNormalMeanScalePrecision rule allocates less than general rule" begin
        ...
        for n in (10, 100)
            ...
            m_μ_general = MvNormalMeanCovariance(
                zeros(n), diageye(Float64, n) / 3.0
            )
            q_γ = GammaShapeRate(2.0, 1.0)
            ...
            allocs_general = @allocated @call_rule MvNormalMeanScalePrecision(
                :out, Marginalisation
            ) (m_μ = m_μ_general, q_γ = q_γ)
