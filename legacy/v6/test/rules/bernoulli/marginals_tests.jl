...
@testitem "marginalrules:Bernoulli" tags = [:rules] begin
    ...
    @testset "out_p: (m_out::Bernoulli, m_p::PointMass)" begin
        @test_marginalrules [check_type_promotion = true] Bernoulli(:out_p) [
            (
                input = (m_out = Bernoulli(0.8), m_p = PointMass(1.0)),
                output = (out = Bernoulli(1.0), p = PointMass(1.0)),
            ),
            (
                input = (m_out = Bernoulli(0.2), m_p = PointMass(1.0)),
                output = (out = Bernoulli(1.0), p = PointMass(1.0)),
            ),
            (
                input = (m_out = Bernoulli(0.2), m_p = PointMass(0.0)),
                output = (out = Bernoulli(0.0), p = PointMass(0.0)),
            ),
        ]
    end
end
