...
function priorvar(name::Symbol, ::Type{D}; subject::S = Subject(Message{D})) where { S, D }
    return PriorVariable{S, D}(name, subject, PriorVariableProps(), VariableBelief())
end
...
makebelief(priorvar::PriorVariable)   = combineLatest(priorvar.messageout, priorvar.props.messagein, strategy = PushNewBut{1}()) |> reduce_to_belief
