...
function score(::BetheFreeEnergy, model::Model, scheduler)
    average_energies = map(filter(isstochastic, getnodes(model))) do node
        ...
        return marginals |> schedule_on(scheduler) |> map(Float64, (m) -> score(AverageEnergy(), functionalform(node), m, metadata(node)) - mapreduce(d -> score(DifferentialEntropy(), d), +, m, init = 0.0)) 
    end
