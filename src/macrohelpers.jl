...
function expression_convert_eltype(eltype::Type{T}, expr::Expr; exceptions = [ :ContinuousUnivariateLogPdf ]) where T
    ...
    return f ∈ exceptions ? expr : :(ReactiveMP.convert_eltype($f{$T}, $expr))
end
