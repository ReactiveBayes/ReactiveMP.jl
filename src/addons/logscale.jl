...
function getlogscale(addons::NTuple{N, AbstractAddon}) where {N}
    ...
    if length(logscales) === 0
        error(
            "Log-scale addon is not available. Make sure to include AddonLogScale in the addons.",
        )
    end
...
function message_mapping_addon(
    ::AddonLogScale{Nothing}, mapping, messages, marginals, result::Distribution
)
    # Here we assume
    ...
    elseif isnothing(messages) && all(data -> data isa PointMass, marginals)
        ...
        error(
            "Log-scale value has not been computed for the message update rule = $(mapping)",
        )
    end
end
...
function multiply_addons(
    left_addon::AddonLogScale{Missing},
...
function multiply_addons(
    ...
    right_addon::AddonLogScale{Missing},
    new_dist,
    left_dist,
    right_dist::Missing,
)
