...
function multiply_addons(left_addon::AddonMemory, right_addon::AddonMemory, new_dist, left_dist, right_dist)
    return AddonMemory(update_memory(getmemory(left_addon), getmemory(right_addon)))
end

function update_memory(left::AddonMemoryMessageMapping, right::AddonMemoryMessageMapping)
    return AddonMemoryProd(Any[left, right])
end

function update_memory(left::AddonMemoryMessageMapping, right::AddonMemoryProd)
    pushfirst!(right.mappings, left)
    return right
end

function update_memory(left::AddonMemoryProd, right::AddonMemoryMessageMapping)
    push!(left.mappings, right)
    return left
end

function update_memory(left::AddonMemoryProd, right::AddonMemoryProd)
