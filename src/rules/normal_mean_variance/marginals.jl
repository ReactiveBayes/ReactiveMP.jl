...
@marginalrule NormalMeanVariance(:out_μ_v) (m_out::NormalDistributionsFamily, m_μ::Dirac, m_v::Dirac) = begin
    return (out = prod(ProdPreserveParametrisation(), NormalMeanVariance(mean(m_μ), mean(m_v)), m_out), μ = m_μ, v = m_v)
end

@marginalrule NormalMeanVariance(:out_μ_v) (m_out::Dirac, m_μ::NormalDistributionsFamily, m_v::Dirac) = begin
