...
@marginalrule DiscreteTransition(:out_in) (m_out::Categorical, m_in::Categorical, q_a::DirichletCollection{T, 2, A}) where {T, A} = begin
...
@marginalrule DiscreteTransition(:out_in) (m_out::Categorical, m_in::Categorical, q_a::PointMass) = begin
    B = Diagonal(probvec(m_out)) * mean(q_a) * Diagonal(probvec(m_in))
    P = map!(Base.Fix2(/, sum(B)), B, B) # inplace version of B ./ sum(B)
    return Contingency(P, Val(false))    # Matrix `P` has been normalized by hand
end

@marginalrule DiscreteTransition(:out_in_a) (m_out::Categorical, m_in::Categorical, m_a::PointMass) = begin
...
@marginalrule DiscreteTransition(:out_in_a) (m_out::PointMass, m_in::Categorical, m_a::PointMass, meta::Any) = begin
    m_in_2 = @call_rule DiscreteTransition(:in, Marginalisation) (m_out = m_out, m_a = m_a, meta = meta)
    return convert_paramfloattype((out = m_out, in = prod(ClosedProd(), m_in_2, m_in), a = m_a))
end

@marginalrule DiscreteTransition(:out_in) (m_out::PointMass, m_in::Categorical, q_a::PointMass) = begin
    m_in_2 = @call_rule DiscreteTransition(:in, Marginalisation) (m_out = m_out, q_a = q_a)
...
function marginalrule(
    ::Type{<:DiscreteTransition}, ::Val{marginal_symbol}, ::Val{message_names}, messages::Tuple, ::Val{marginal_names}, marginals::Tuple, ::Any, ::Any
