...
@rule DiscreteTransition(:in, Marginalisation) (
    q_out::PointMass{<:AbstractVector},
    q_a::PointMass{<:AbstractArray{T, 4}},
    m_T1::DiscreteNonParametric,
    m_T2::DiscreteNonParametric,
    meta::Any,
) where {T} = begin
    eloga = mean(Base.Broadcast.BroadcastFunction(clamplog), q_a)
...
@rule DiscreteTransition(:in, Marginalisation) (
    q_out::PointMass{<:AbstractVector},
    q_a::PointMass{<:AbstractArray{T, 5}},
    m_T1::DiscreteNonParametric,
    m_T2::DiscreteNonParametric,
    m_T3::DiscreteNonParametric,
    meta::Any,
) where {T} = begin
    eloga = mean(Base.Broadcast.BroadcastFunction(clamplog), q_a)
    ...
    softmax!(out)
    @tullio msg[i] :=
        out[i, j, k, l] * probvec(m_T1)[j] * probvec(m_T2)[k] * probvec(m_T3)[l]
    return Categorical(normalize!(msg, 1); check_args = false)
end
...
@rule DiscreteTransition(:T1, Marginalisation) (
    ...
    softmax!(out)
    @tullio msg[j] :=
        out[i, j, k, l] * probvec(m_in)[i] * probvec(m_T2)[k] * probvec(m_T3)[l]
    return Categorical(normalize!(msg, 1); check_args = false)
end
...
@rule DiscreteTransition(:T2, Marginalisation) (
    ...
    softmax!(out)
    @tullio msg[k] :=
        out[i, j, k, l] * probvec(m_in)[i] * probvec(m_T1)[j] * probvec(m_T2)[l]
    return Categorical(normalize!(msg, 1); check_args = false)
end
...
@rule DiscreteTransition(:T3, Marginalisation) (
    ...
    softmax!(out)
    @tullio msg[l] :=
        out[i, j, k, l] * probvec(m_in)[i] * probvec(m_T1)[j] * probvec(m_T2)[k]
    return Categorical(normalize!(msg, 1); check_args = false)
end
