...
@rule softdot(:γ, Marginalisation) (
    q_y_x::MultivariateNormalDistributionsFamily, q_θ::Any
) = begin
    ...
    mx, Vx = ar_slice(F, y_x_mean, 2:(order + 1)),
    ar_slice(F, y_x_cov, 2:(order + 1), 2:(order + 1))
    ...
    B =
        first(R) - 2 * first(mθ' * L) + first(mθ' * C * mθ) + mul_trace(Vθ, C)
