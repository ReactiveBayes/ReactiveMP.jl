...
@rule Binomial(:k, Marginalisation) (q_n::Any, q_p::Any) = begin
...
@rule Binomial(:k, Marginalisation) (m_n::Any, m_p::Any) = begin
    supp_n = if typeof(m_n) <: ExponentialFamilyDistribution
        DomainSets.ClosedInterval(minimum(getsupport(m_n)):maximum(getsupport(m_n)))
    else
        DomainSets.ClosedInterval(minimum(Distributions.support(m_n)):maximum(Distributions.support(m_n)))
    end
