...
@rule MvNormalMeanScalePrecision(:out, Marginalisation) (q_μ::Any, q_γ::Any) =
    MvNormalMeanPrecision(mean(q_μ), mean(q_γ) * diageye(ndims(q_out)))

@rule MvNormalMeanScalePrecision(:out, Marginalisation) (m_μ::PointMass, q_γ::Any) =
    MvNormalMeanPrecision(mean(m_μ), mean(q_γ) * diageye(ndims(q_out)))
