...
@rule DeltaFn(:out, Marginalisation) (
    q_ins::FactorizedJoint{P}, meta::DeltaMeta{M}
) where {P <: NTuple{1}, M <: CVI} = begin
...
@rule DeltaFn(:out, Marginalisation) (
    q_ins::FactorizedJoint, meta::DeltaMeta{M}
) where {M <: CVI} = begin
    method = ReactiveMP.getmethod(meta)
    q_ins_sample_friendly = map(
        marginal -> sampling_optimized(marginal), components(q_ins)
    )
    rng = something(method.rng, Random.default_rng())
    q_ins_samples = map(
        marginal -> rand(rng, marginal, method.n_samples),
        q_ins_sample_friendly,
    )
