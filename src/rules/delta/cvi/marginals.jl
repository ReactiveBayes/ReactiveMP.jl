...
@marginalrule DeltaFn{f}(:ins) (m_out::Any, m_ins::ManyOf{1, Any}, meta::DeltaMeta{M}) where {f, M <: CVIApproximation} = begin
    q = convert(Distribution, render_cvi(getmethod(meta), (z) -> logpdf(m_out, f(z)), first(m_ins)))
    return FactorizedJoint((q,))
end

@marginalrule DeltaFn{f}(:ins) (m_out::Any, m_ins::ManyOf{N, Any}, meta::DeltaMeta{M}) where {f, N, M <: CVIApproximation} = begin
