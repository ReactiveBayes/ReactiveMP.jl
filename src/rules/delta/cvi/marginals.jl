...
@marginalrule DeltaFn(:ins) (
    ...
    F = promote_variate_type(
        variate_form(typeof(first(m_ins))), AbstractContinuousGenericLogPdf
    )
...
@marginalrule DeltaFn(:ins) (
    ...
    pre_samples = zip(
        map(
            m_in_k -> cvilinearize(rand(rng, m_in_k, method.n_samples)),
            m_ins
        )...
    )
    ...
    logp_nc_drop_index =
        let g = getnodefn(meta, Val(:out)), pre_samples = pre_samples
            (z, i, pre_samples) -> begin
                samples = map(
                    ttuple -> TupleTools.insertat(ttuple, i, (z,)),
                    pre_samples
                )
            ...
            (i, pre_samples) -> begin
                ...
                logp = convert(
                    promote_variate_type(
                        variate_form(typeof(first(m_ins))),
