...
@rule GaussianProcess(:params, Marginalisation) (q_out::GaussianProcess, q_meanfunc::PointMass, q_kernelfunc::PointMass, 
    ...
    g = (x) -> inv_cov_mat(cov_strategy, kernelfunc, train, Σ, x, inducing)

    logp_nc = (x) -> -1/2 * (y - meanf.(train))' * g(x)*(y - meanf.(train)) + 1/2 * logdet(g(x)) - length(train)/2 * log(2π)
...
function inv_cov_mat(::CovarianceMatrixStrategy{<:FullCovarianceStrategy}, kernfunc::Kernel, input::Array{Float64}, Σ::AbstractArray, θ, inducing::Array{Float64})
    return inv(kernelmatrix(kernfunc(exp.(θ)),input,input) + Diagonal(Σ) + 1e-8*diageye(length(input)))
end

function inv_cov_mat(::CovarianceMatrixStrategy{<:DeterministicInducingConditional}, kernfunc::Kernel, input::Array{Float64}, Σ::AbstractArray, θ, inducing::Array{Float64})
...
function inv_cov_mat(::CovarianceMatrixStrategy{<:DeterministicTrainingConditional}, kernfunc::Kernel, input::AbstractArray, Σ::AbstractArray, θ, inducing::AbstractArray)
    Kuu = kernelmatrix(kernfunc(exp.(θ)),inducing,inducing) + 1e-5*diageye(length(inducing))
    Kfu = kernelmatrix(kernfunc(exp.(θ)),input,inducing)
    Λ = Diagonal(Σ)
