...
@rule NormalMixture((:m, k), Marginalisation) (
    q_out::Any,
    q_switch::Union{Categorical, Bernoulli, PointMass{<:AbstractVector}},
    q_p::Any
) = begin
    pv    = probvec(q_switch)
    T     = eltype(pv)
    ...
    return convert(
        promote_variate_type(F, NormalMeanPrecision),
        mean(q_out),
        z_bar[k] * mean(q_p)
    )
end
...
@rule NormalMixture((:m, k), Marginalisation) (
    q_out::Any, q_switch::PointMass{<:Real}, q_p::Any
) = begin
    error(
