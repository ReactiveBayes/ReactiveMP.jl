...
logger_pipeline_stage_println(logger::LoggerPipelineStage, something::Any) = logger_pipeline_stage_println(
    logger,
    logger.output,
    logger_pipeline_stage_append_prefix(logger, something),
)

logger_pipeline_stage_println(logger::LoggerPipelineStage, output::Core.CoreSTDOUT, something) = Core.println(
    output, something
)
logger_pipeline_stage_println(logger::LoggerPipelineStage, output, something) = println(
    output, something
)

logger_pipeline_stage_append_prefix(logger::LoggerPipelineStage, something) = string(
    "[", logger.prefix, "]: ", something
)
