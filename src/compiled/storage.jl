...
CompiledPackedPool(::Type{M}, ::Type{D}, n) where {M, D} =
    CompiledPackedPool{M, D}(Vector{D}(undef, n), zeros(UInt8, n))
...
@inline function compiled_unpack(
    pool::CompiledPackedPool{M}, i
) where {M <: Message}
    flags = pool.flags[i]
...
function Base.getindex(values::CompiledValues, i::Int)
    tag = values.tags[i]
    return if iszero(tag)
        values.boxed[i]
    else
        compiled_unpack(values.pools[tag], values.offsets[i])
    end
end
@inline function compiled_pack!(pool::CompiledPackedPool{M}, i, value) where {M}
    if value isa M && isempty(getannotations(value))
        ...
        pool.flags[i] =
            UInt8(is_clamped(value)) | (UInt8(is_initial(value)) << 1)
        return true
    end
    return false
end
function Base.setindex!(values::CompiledValues, value, i::Int)
    tag = values.tags[i]
    if iszero(tag) ||
        !compiled_pack!(values.pools[tag], values.offsets[i], value)
        values.tags[i] = 0
        values.boxed[i] = value
    end
    return value
end
compiled_ready(values::CompiledValues, slot::Int32) =
    !iszero(values.tags[slot]) || values.boxed[slot] !== nothing

compiled_packable(::Any) = false
compiled_packable(value::Union{Message{D}, Marginal{D}}) where {D} =
    isbitstype(D) && isempty(getannotations(value))
...
function pack_compiled_values(boxed::Vector{Any})
    ...
    storage = CompiledValues(
        boxed, zeros(UInt16, length(boxed)), zeros(Int32, length(boxed)), pools
    )
...
function compiled_accessor(values::CompiledValues, slot::Int32)
    ...
    return if iszero(tag)
        CompiledBoxedAccess{typeof(values.boxed[slot])}()
    else
        CompiledPackedAccess{typeof(values.pools[tag])}(tag)
    end
end
compiled_accessor(values, ::Nothing) = nothing
compiled_accessor(values, slots::Tuple) =
    map(slot -> compiled_accessor(values, slot), slots)
compiled_accessor(values, slots::CompiledManyOf) =
    CompiledManyAccess(compiled_accessor(values, slots.slots))
...
compiled_specializable(accessors::Tuple) =
    all(compiled_specializable, accessors)
compiled_specializable(accessor::CompiledManyAccess) =
    compiled_specializable(accessor.inner)

@inline compiled_access_valid(
    values, slot::Int32, access::CompiledPackedAccess
) = values.tags[slot] == access.pool
@inline compiled_access_valid(
    values, slot::Int32, ::CompiledBoxedAccess{T}
) where {T} = iszero(values.tags[slot]) && values.boxed[slot] isa T
@inline compiled_access_valid(values, ::Nothing, ::Nothing) = true
@inline compiled_access_valid(values, slots::Tuple, access::Tuple) =
    all(map((slot, a) -> compiled_access_valid(values, slot, a), slots, access))
@inline compiled_access_valid(
    values, slots::CompiledManyOf, access::CompiledManyAccess
) = compiled_access_valid(values, slots.slots, access.inner)

@inline function compiled_access_read(
    values, slot::Int32, access::CompiledPackedAccess{P}
) where {P}
    pool = values.pools[access.pool]::P
    return compiled_unpack(pool, values.offsets[slot])
end
@inline compiled_access_read(
    values, slot::Int32, ::CompiledBoxedAccess{T}
) where {T} = values.boxed[slot]::T
@inline compiled_access_read(values, ::Nothing, ::Nothing) = nothing
@inline compiled_access_read(values, slots::Tuple, access::Tuple) =
    map((slot, a) -> compiled_access_read(values, slot, a), slots, access)
@inline compiled_access_read(
    values, slots::CompiledManyOf, access::CompiledManyAccess
) = ManyOf(compiled_access_read(values, slots.slots, access.inner))

@inline function compiled_access_write!(
    values, slot, access::CompiledPackedAccess{P}, result
) where {P}
    if values.tags[slot] == access.pool && compiled_pack!(
        values.pools[access.pool]::P, values.offsets[slot], result
    )
        return nothing
    end
    values[slot] = result
...
@inline function compiled_access_write!(
    values, slot, ::CompiledBoxedAccess, result
)
    values[slot] = result
    return nothing
end
...
@noinline function compiled_typed_evaluate!(
    executor, kernel, values, bindings, output
)
...
function execute_compiled_typed!(plan::CompiledTypedPlan, program, index)
    ...
    return compiled_typed_evaluate!(
        plan.executors[id],
        program.kernels[operation.kernel],
        program.values,
        operation.inputs,
        operation.output,
    )
