...
struct ImportanceSamplingApproximation{T, R}
    ...
    bsamples     :: Vector{T}
    btransformed :: Vector{T}
    bweights     :: Vector{T}
end

ImportanceSamplingApproximation(rng::R, nsamples::Int)            where { R }    = ImportanceSamplingApproximation(Float64, rng, nsamples) 
ImportanceSamplingApproximation(::Type{T}, rng::R, nsamples::Int) where { T, R } = ImportanceSamplingApproximation{T, R}(rng, nsamples, Vector{T}(undef, nsamples), Vector{T}(undef, nsamples), Vector{T}(undef, nsamples))
...
function approximate_meancov(approximation::ImportanceSamplingApproximation, g::Function, distribution)
    ...
    map!(g, approximation.btransformed, approximation.bsamples)

    normalization = sum(approximation.btransformed)

    map!(Base.Fix2(/, normalization), approximation.bweights, approximation.btransformed)
