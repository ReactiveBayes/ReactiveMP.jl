...
@node ComplexNormal Stochastic [ out, (μ, aliases = [ mean ]), (Γ, aliases = [ invcov, precision ]), (C, aliases = [ relation ]) ]
...
@average_energy ComplexNormal (q_out::Any, q_μ::Any, q_Γ::Any, q_C::Any) = begin
    ...
    return logπ + log(Γ_mean) + Γ_mean * (μ_var + out_var + abs2(μ_mean - out_mean))
end