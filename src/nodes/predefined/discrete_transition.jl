...
@average_energy DiscreteTransition (q_out::Any, q_in::Any, q_a::DirichletCollection{T, 2, A}, meta::Any) where {T, A} = begin
    return -probvec(q_out)' * mean(BroadcastFunction(log), q_a) * probvec(q_in)
end

@average_energy DiscreteTransition (q_out_in::Contingency, q_a::DirichletCollection{T, 2, A}, meta::Any) where {T, A} = begin
    return -tr(components(q_out_in)' * mean(BroadcastFunction(log), q_a))
end

@average_energy DiscreteTransition (q_out_in::Contingency, q_a::PointMass{<:AbstractArray{T, 2}}, meta::Any) where {T} = begin
...
@average_energy DiscreteTransition (q_out::PointMass{<:AbstractVector}, q_in::Categorical, q_a::PointMass{<:AbstractArray{T, 2}}, meta::Any) where {T} = begin
    return -probvec(q_out)' * mean(BroadcastFunction(clamplog), q_a) * probvec(q_in)
end

function score(
    ::AverageEnergy, ::Type{<:DiscreteTransition}, ::Val{mnames}, marginals::Tuple{<:Marginal{<:Contingency}, <:Marginal{<:DirichletCollection}}, meta::Any
) where {mnames}
