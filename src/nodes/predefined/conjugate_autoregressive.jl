...
identical to `AR` — `y₁ ∼ N(θᵀx, γ⁻¹)` with companion-form state propagation — but
...
mean-field edges `θ` and `γ` of `AR`.
...
are the expected sufficient statistics of `q(y, x)`. The node reuses `ARMeta` for
