...
@testitem "rule" begin
    ...
    @testset "Testing utilities" begin
        ...
        @testset "TestRulesConfiguration" begin
            ...
            let configuration = TestRulesConfiguration()
                for atol in (1e-6, 1e-12)
                    ...
                    @test all(
                        ...
                        values(float_tolerance(configuration))
                    )
                end
            end
            ...
            let configuration = TestRulesConfiguration()
                for atol in [
                    ...
                    for (key, value) in atol
                        @test isequal(
                            float_tolerance(configuration, key), value
                        )
                    end
                end
            end
            ...
            @test_throws ErrorException test_rules_parse_configuration(
                :configuration, :([1])
            )
            @test_throws ErrorException test_rules_parse_configuration(
                :configuration, :([options = value = broken])
            )
            ...
            for name in (:configuration, :blabla),
                ...
                @test inexpr(
                    expression,
                    :(ReactiveMP.extra_float_types!($name, $extra_types))
                )
            end
        end
        ...
        @testset "test_rules_generate_testset" begin
            ...
            tfns = (:f, :test)
            specs = (
                :(NormalMeanVariance(:out, Marginalisation)),
            ...
            for f in fns,
                ...
                expression = test_rules_generate_testset(
                    test_entry, test_f, f, spec, :configuration
                )
        ...
        @testset "TestRuleEntryInputSpecification" begin
            ...
            let spec = TestRuleEntryInputSpecification(
                    [:q_x => 1, :m_y => :(Normal(1.0, 1.0))], :(Meta(2))
                )
                @test rule_macro_convert_to_expr(spec) ==
                    :((q_x = 1, m_y = Normal(1.0, 1.0), meta = Meta(2)))
            end
        end
        ...
        @testset "convert(TestRuleEntryInputSpecification, expression)" begin
            ...
            @test convert(
                ...
                [:key1 => 1, :key2 => 3, :key3 => 2], nothing
            )
            @test convert(
                TestRuleEntryInputSpecification,
                :((key1 = 1, key2 = 2, key3 = 3))
            ) == TestRuleEntryInputSpecification(
                [:key1 => 1, :key2 => 2, :key3 => 3], nothing
            )
            @test convert(
                TestRuleEntryInputSpecification,
                :((
            ...
            @test convert(
                TestRuleEntryInputSpecification,
                :((key1 = 1, key2 = 2, key3 = 3, meta = "hello"))
            ) == TestRuleEntryInputSpecification(
                [:key1 => 1, :key2 => 2, :key3 => 3], :("hello")
            )
            @test convert(
                TestRuleEntryInputSpecification,
                :((key1 = 1, key2 = ManyOf(1, 2), key3 = 3, meta = "hello"))
            ) == TestRuleEntryInputSpecification(
                [:key1 => 1, :key2 => :(ManyOf(1, 2)), :key3 => 3], :("hello")
            )
        end
        ...
        @testset "TestRuleEntry" begin
            ...
            let spec = TestRuleEntry(
                    TestRuleEntryInputSpecification([:m_x => 1], nothing),
                    :(Normal(0.0, 3.0))
                )
                @test rule_macro_convert_to_expr(spec) ==
                    :((input = (m_x = 1,), output = Normal(0.0, 3.0)))
            end
        ...
        @testset "convert(TestRuleEntry, expression)" begin
            ...
            @test_throws ErrorException convert(
                TestRuleEntry, :([input, output])
            )
            @test_throws ErrorException convert(
                TestRuleEntry, :([input = 2, output])
            )
            @test_throws ErrorException convert(
                TestRuleEntry, :([input = 2, output])
            )
            ...
            let entry = convert(
                    TestRuleEntry,
                    :(input = (m_x = Normal(3.0, 3.0),), output = 3)
                )
            ...
            let entries = convert(
                ...
                @test inexpr(
                    entries[1].input.arguments[1][2], :(Normal(1.0, 2.0))
                )
                ...
                @test inexpr(
                    entries[2].input.arguments[1][2], :(Normal(2.0, 3.0))
                )
        ...
        @testset "test_rules_convert_paramfloattype_for_test_entry" begin
            ...
            for m in (1, :(Normal(0.0, 1.0))),
                ...
                let test_entry = TestRuleEntry(
                    ...
                    modified_inputs = map(
                        e -> rule_macro_convert_to_expr(e),
                        test_rules_convert_paramfloattype_for_test_entry(
                            test_entry, eltype
                        )
                    )
        ...
        @testset "`@rule` invalid input arguments" begin
            ...
            @eval @rule MyNode(:out, Marginalisation) (
                m_a::PointMass, q_b::PointMass, meta::Int
            ) = begin end
            @test_throws LoadError @eval @rule MyNode(:out, Marginalisation) (
        ...
        @testset "`@marginalrule` invalid input arguments" begin
            ...
            @eval @marginalrule MyNode(:a_b) (
                m_a::PointMass, m_b::PointMass, q_c::PointMass, meta::Int
            ) = begin end
            @test_throws LoadError @eval @marginalrule MyNode(:out) (
        ...
        @testset "basic `test_rule` macro invokation" begin
            ...
            @rule TestNodeForTestRuleMacro(:out, Marginalisation) (m_x::PointMass, q_y::PointMass) = PointMass(
                mean(m_x) + mean(q_y)
            )
            @rule TestNodeForTestRuleMacro(:out, Marginalisation) (m_x::ManyOf{N, Any}, q_y::PointMass) where {N} = PointMass(
                sum(mean, m_x) + mean(q_y)
            )
            @rule TestNodeForTestRuleMacro(:out, Marginalisation) (m_x::PointMass, q_y::PointMass, meta::typeof(-)) = PointMass(
                mean(m_x) - mean(q_y)
            )

            ReactiveMP.@test_rules [check_type_promotion = true] TestNodeForTestRuleMacro(
                :out, Marginalisation
            ) [
                (
                    input = (m_x = PointMass(1), q_y = PointMass(2)),
                    output = PointMass(3)
                ),
                (
                    input = (m_x = PointMass(1), q_y = PointMass(2)),
                ...
                (
                    input = (
                        m_x = ManyOf(PointMass(1), PointMass(1)),
                        q_y = PointMass(2)
                    ),
                    output = PointMass(4)
                ),
                (
                    input = (
                        m_x = ManyOf(PointMass(1), PointMass(2)),
                        q_y = PointMass(2)
                    ),
                    output = PointMass(5)
                ),
                (
                    input = (m_x = PointMass(1), q_y = PointMass(2), meta = -),
                    output = PointMass(-1)
                ),
                (
                    input = (m_x = PointMass(3), q_y = PointMass(2), meta = -),
                    output = PointMass(1)
                )
            ]
            ...
            @rule TestNodeForTestRuleMacro(:out, Marginalisation) (m_x::PointMass, q_y::PointMass, meta::TestMetaForFailingRule) = PointMass(
                1.0
            )
    ...
    @testset "Macro utilities" begin
        import ReactiveMP:
            rule_macro_parse_on_tag,
            rule_macro_parse_fn_args,
            call_rule_macro_parse_fn_args,
            rule_macro_check_fn_args

        @testset "rule_macro_parse_on_tag(expression)" begin
            @test rule_macro_parse_on_tag(:(:out)) ==
                (:(Val{:out}), nothing, nothing)
            @test rule_macro_parse_on_tag(:(:mean)) ==
                (:(Val{:mean}), nothing, nothing)
            @test rule_macro_parse_on_tag(:(:mean, k)) ==
                (:(Tuple{Val{:mean}, Int}), :k, :(k = on[2]))
            @test rule_macro_parse_on_tag(:(:precision, r)) ==
                (:(Tuple{Val{:precision}, Int}), :r, :(r = on[2]))
        ...
        @testset "rule_macro_check_fn_args(inputs; allowed_inputs, allowed_prefixes)" begin
            ...
            @test_throws ErrorException rule_macro_check_fn_args(
                [(:a, 1), (:b, 2)];
                allowed_inputs = (),
                allowed_prefixes = (:m_, :q_)
            )
            @test_throws ErrorException rule_macro_check_fn_args(
                [(:meta, 3)]; allowed_inputs = (), allowed_prefixes = (:m_, :q_)
            )
        end
        ...
        @testset "call_rule_macro_parse_fn_args(inputs; specname, prefix, proxy)" begin
            names, values = call_rule_macro_parse_fn_args(
                [
                    (:m_out, :(PointMass(1.0))),
                    (:m_mean, :(NormalMeanPrecision(0.0, 1.0)))
                ];
                specname = :messages,
                prefix = :m_,
                proxy = :(ReactiveMP.Message)
            ...
            names, values = call_rule_macro_parse_fn_args(
                [
                    (:m_out, :(PointMass(1.0))),
                    (:q_mean, :(NormalMeanPrecision(0.0, 1.0)))
                ];
                specname = :marginals,
                prefix = :q_,
                proxy = :(ReactiveMP.Marginal)
    ...
    @testset "Error utilities" begin
        @testset "rule_method_error" begin
            ...
            let
                ...
                @test occursin(
                    "q_β::ExponentialFamily.GammaDistributionsFamily", output
                )
            end
            ...
            let
                ...
                @test occursin("q_a::BayesBase.PointMass", output)
                @test occursin(
                    "MyCustomNode(m_a::BayesBase.PointMass, q_a::BayesBase.PointMass, meta::Float64)",
                    output
                )
                @test occursin("meta::Float64", output)
            end
        ...
        @testset "get_from_rule_method" begin
            let
                ...
                marginals_rule1 = ReactiveMP.get_marginals_from_rule_method(
                    rule1
                )
                marginal_names_rule1 = ReactiveMP.get_marginal_names_from_rule_method(
                    rule1
                )
                marginal_types_rule1 = ReactiveMP.get_marginal_types_from_rule_method(
                    rule1
                )

                @test occursin(
                    "GetFromRuleMethod",
                    ReactiveMP.get_node_from_rule_method(rule1)
                )
                @test occursin(
                    "μ(in) :: ExponentialFamily.NormalMeanVariance",
                    messages_rule1[1]
                )
                ...
                @test occursin(
                    "q(a) :: ExponentialFamily.NormalMeanPrecision",
                    marginals_rule1[1]
                )
    ...
    @testset "Check that default meta is `nothing`" begin
        ...
        @rule DummyNode(:out, Marginalisation) (
        ...
        @test_throws ReactiveMP.RuleMethodError (@call_rule DummyNode(
            :out, Marginalisation
        ) (
            q_x = vague(NormalMeanPrecision),
            q_y = vague(NormalMeanPrecision),
