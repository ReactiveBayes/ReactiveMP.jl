@testitem "Basic checks for marginal rule" begin
    ...
    @testset "f(x) -> x, x~EF, out~EF" begin
        ...
        inputs = [
            ...
            (
                ...
                MvNormalMeanCovariance([0.5, 2.5, -3.0]),
            ),
        ]
        for input in inputs
            m_in = input[1]
    ...
    @testset "f(x) -> x, x~EF, out~EF with Binomial" begin
        ...
        for input_output in inputs_outputs
            ...
            q_prod = prod(
                PreserveTypeProd(ExponentialFamilyDistribution), m_in, m_out
            )
    ...
    @testset "f(x) -> x, x~EF, out~EF with Categorical" begin
        meta = DeltaMeta(
            method = CVIProjection(
                in_prjparams = (
                    in_1 = ExponentialFamilyProjection.ProjectionParameters(
        ...
        inputs_outputs = [
            (
                Categorical([1 / 4, 1 / 4, 1 / 2]),
                Categorical([1 / 2, 1 / 8, 3 / 8]),
            ),
            (
                Categorical([1 / 2, 1 / 8, 3 / 8]),
                Categorical([1 / 16, 13 / 16, 1 / 8]),
            ),
            (
                Categorical([1 / 8, 1 / 8, 1 / 8, 5 / 8]),
                Categorical([2 / 7, 2 / 7, 1 / 7, 2 / 7]),
            ),
        ]
        for input_output in inputs_outputs
            m_in = first(input_output)
...
@testitem "CVIProjection form access tests" begin
    ...
    @testset "Testing input edge form access with get_kth_in_form" begin
        ...
        method_with_forms = CVIProjection(
            in_prjparams = (in_1 = form1, in_2 = form2)
        )
...
@testitem "CVIProjection proposal distribution convergence tests" begin
    ...
    @testset "Posterior approximation quality" begin
        ...
        method = CVIProjection(rng = rng, sampling_strategy = FullSampling(500))
