@testitem "Basic out rule tests #1" begin
    ...
    @testset "f(x) = x, x ~ EF" begin
        ...
        q_ins_m_out_incomings = [
            (
                FactorizedJoint((NormalMeanVariance(2, 2),)),
...
@testitem "Basic out rule tests #2" begin
    ...
    meta = DeltaMeta(
        method = CVIProjection(outsamples = 1000), inverse = nothing
    )
    f(x, y) = [x; y]
    q_ins_m_out_incomings = [
        (
...
@testitem "Basic out rule tests #3" begin
    ...
    @testset "f(x) = a*x + b, x ~ Normal (Univariate)" begin
        meta = DeltaMeta(
            method = CVIProjection(outsamples = 10000), inverse = nothing
        )
        q_ins = [
        ...
        for q_in in q_ins,
            ...
            f = (x) -> a * x + b
            q_in_component = first(components(q_in))
            q_out = NormalMeanVariance(
                a * mean(q_in_component) + b, a^2 * var(q_in_component)
            )
    ...
    @testset "f(x) = x + constant, x ~ Normal (Multivariate)" begin
        ...
        for (q_in, m_out_incoming, f, c) in q_ins_m_out_incomings_fs_constants
            ...
            q_out = MvNormalMeanCovariance(
                mean(q_in_component) + c, cov(q_in_component)
            )
...
@testitem "Basic out rule tests #5" begin
    ...
    @testset "Differen set of non-linearities x ~ NonNormal EF (Univariate)" begin
        ...
        q_ins_m_out_incomings_q_outs_non_linearities = [
            (
                FactorizedJoint((Beta(5, 2),)),
                Beta(20, 3),
                Beta(2, 5),
                x -> 1 - x
            ),
            (
                FactorizedJoint((Gamma(3, 4),)),
                Gamma(10, 7),
                Gamma(3, 4 * 0.1),
                x -> 0.1 * x
            ),
            (
                FactorizedJoint((Exponential(0.5),)),
                Exponential(3),
                Rayleigh(1 / (sqrt(2 * 3))),
                x -> sqrt(x)
            ),
        ...
        for (q_in, m_out_incoming, q_out, f) in
            q_ins_m_out_incomings_q_outs_non_linearities
...
@testitem "ProjectionForm target tests" begin
    ...
    @testset "Testing abs transformation with ProjectedTo override" begin
        ...
        meta = DeltaMeta(
            method = CVIProjection(out_prjparams = form, outsamples = 1000),
            inverse = nothing
        )
