module FlowNodeNeuralNetworksRadialMapTest
...
@testset "Radial Map" begin
    ...
    @testset "Constructor" begin
        ...
        # wrong constructors
        @test_throws MethodError RadialMap(1.0, 2.0, [2.0, 3.0])
        @test_throws MethodError RadialMap(0.5, 1.0, [5.0, 6.0])
        @test_throws MethodError RadialMap(2.0, [2.0, 3.0], 1.0)
        @test_throws MethodError RadialMap(1.0, [5.0, 6.0], 0.5)
        @test_throws AssertionError RadialMap(1.0, -2.0, 1.0)
        @test_throws AssertionError RadialMap(2.0, -1.0, -2.0)
        @test_throws AssertionError RadialMap([2.0, 3.0], -1.0, 1.0)
        @test_throws AssertionError RadialMap([5.0, 9.0], -5.0, -2.0)
        
    end

    @testset "Get-Set" begin
        
        # check get functions for univariate RadialMap
        f = RadialMap(1.0, 2.0, 3.0)
        @test getz0(f)  == f.z0
        @test getα(f)   == f.α
        @test getβ(f)   == f.β
        ...
        # check get functions for multivariate RadialMap
        f = RadialMap([1.0, 2.0], 3.0, 4.0)
        @test getz0(f)  == f.z0
        @test getα(f)   == f.α
        @test getβ(f)   == f.β
        @test getall(f) == (f.z0, f.α, f.β)
    ...
    @testset "Base" begin
        ...
        @test eltype(RadialMap{Float64,Float64}) == Float64
        @test size(f) == 1
        @test length(f) == 1
        ...
        f = RadialMap([1.0, 2.0], 3.0, 5.0)
        @test eltype(f) == Float64
        @test eltype(RadialMap{Array{Float64,1},Float64}) == Float64
        @test size(f) == 2
        @test length(f) == 2
