...
@testset "InvWishartNode" begin
    @testset "Creation" begin
        node = make_node(InvWishart)

        @test functionalform(node) === InvWishart
        ...
        node = make_node(InvWishart, FactorNodeCreationOptions(nothing, 1, nothing))
    ...
    @testset "AverageEnergy" begin
        begin
            q_out = InvWishart(2.0, [2.0 0.0; 0.0 2.0])
            q_ν   = PointMass(2.0)
            ...
            @test score(AverageEnergy(), InvWishart, Val{(:out, :ν, :S)}, marginals, nothing) ≈
                  9.496544113156787
        end
