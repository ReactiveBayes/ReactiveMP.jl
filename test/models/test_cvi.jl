...
@model function non_linear_dynamics(T, rng, n_iterations, n_samples, learning_rate)
    ...
    x[1] ~ f(z[1]) where {meta = CVIApproximation(n_iterations, n_samples, rng, Descent(learning_rate))}
    y[1] ~ NormalMeanPrecision(x[1], θ)

    for t in 2:T
        z[t] ~ NormalMeanPrecision(z[t-1] + 1, τ)
        x[t] ~ f(z[t]) where {meta = CVIApproximation(n_iterations, n_samples, rng, Descent(learning_rate))}
