...
function hgf_online_inference(data, vmp_iters, real_k, real_w, z_variance, y_variance)
    ...
    model, (zt, xt, y, gcv_node, xt_min_mean, xt_min_var, zt_min_mean, zt_min_var) = hgf_online_model(real_k, real_w, z_variance, y_variance)
...
@testset "Hierarchical Gaussian Filter" begin
    ...
    @testset "Online inference" begin 
        ...
        plot_output      = joinpath(base_output, "hgf_online_plot_$(timestamp).png")
        benchmark_output = joinpath(base_output, "hgf_online_benchmark_$(timestamp).txt")
        ## -------------------------------------------- ##
        ## Create output plots
