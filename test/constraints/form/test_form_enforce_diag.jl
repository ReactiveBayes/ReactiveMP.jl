...
@testset "EnforceDiagFormConstraint" begin
    ...
    @testset "which_forms" begin
        N = MvNormalMeanCovariance(zeros(2), [2.0 1.0; 1.0 2.0])
        W = WishartMessage(3, [2.0 1.0; 1.0 2.0])
        i = InverseWishartMessage(3, [2.0 1.0; 1.0 2.0])

        @test constrain_form(constraint, N) == N
        @test constrain_form(constraint, W) == WishartMessage(3, [2.0 0.0; 0.0 2.0])
        @test constrain_form(constraint, i) == InverseWishartMessage(3, [2.0 0.0; 0.0 2.0])
    end
end
