...
@testitem "MessageMapping should call provided callbacks handler" begin
    ...
    callbacks = (
        before_message_rule_call = (data) -> push!(events, (event = :before_message_rule_call, data = data)),
        after_message_rule_call = (data) -> push!(events, (event = :after_message_rule_call, data = data)),
    )
    ...
    @test events[1].data.mapping.factornode === SomeArbitraryNodeCallbacksTests()
    @test events[1].data.messages === messages
    @test events[1].data.marginals === marginals

    @test events[2].event == :after_message_rule_call
    @test events[2].data.mapping.factornode === SomeArbitraryNodeCallbacksTests()
...
@testmodule MessageProductContextUtils begin
    ...
    function ReactiveMP.invoke_callback(
        handler::SaveOrderOfComputationCallbacks, ::Val{E}, data
    ) where {E}
...
@testitem "Form constraint callbacks with FormConstraintCheckEach" setup = [
    ...
    @testset "CheckEach applies form constraint after each pairwise product" begin
        ...
        # First constraint: before gets Normal(0, 2/3), after.distribution = before, after.result = constrained
        @test handler.events[1].data.distribution ≈ Normal(0, 2 / 3)
        @test handler.events[2].data.distribution ≈ Normal(0, 2 / 3)
        @test handler.events[2].data.result ≈ Normal(1, 2 / 3)

        # Second constraint
        @test handler.events[3].data.distribution ≈ Normal(9 / 11, 6 / 11)
        @test handler.events[4].data.distribution ≈ Normal(9 / 11, 6 / 11)
...
@testitem "Form constraint callbacks with FormConstraintCheckLast" setup = [
    ...
    @testset "CheckLast applies form constraint once at the end" begin
        ...
        # Before constraint: distribution = Normal(0, 6/11)
        @test handler.events[1].data.distribution ≈ Normal(0, 6 / 11)
        # After constraint: distribution = before, result = constrained Normal(1, 6/11)
        @test handler.events[2].data.distribution ≈ Normal(0, 6 / 11)
        @test handler.events[2].data.result ≈ Normal(1, 6 / 11)

        # The final result matches the after-constraint result
