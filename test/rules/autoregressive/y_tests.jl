...
@testitem "rules:Autoregressive:y" begin
    ...
    @testset "Mean-field: (q_x::Any, q_θ::Any, q_γ::Any)" begin
        ...
        @test_rules [check_type_promotion = true] Autoregressive(
            ...
            (
                input = (
                    q_x = NormalWeightedMeanPrecision(1.0, 1.0),
        ...
        @test_rules [check_type_promotion = true] Autoregressive(
            ...
            (
                ...
                output = MvNormalMeanCovariance(
                    zeros(2), ARTransitionMatrix(order, 1.0)
                )
            ),
            (
                input = (
                ...
                output = MvNormalMeanCovariance(
                    [0.0, 1.0], ARTransitionMatrix(order, 1.0)
                )
            ),
            (
                input = (
                ...
                output = MvNormalMeanCovariance(
                    [2.0, 1.0], ARTransitionMatrix(order, 2.0)
                )
            )
        ]
    end
    ...
    @testset "Structured: (m_x::UnivariateNormalDistributionsFamily, q_θ::UnivariateNormalDistributionsFamily, q_γ::Any)" begin
        ...
        @test_throws MethodError @call_rule AR(:y, Marginalisation) (
            m_x = MvNormalMeanPrecision([0.0], [1.0;]),
