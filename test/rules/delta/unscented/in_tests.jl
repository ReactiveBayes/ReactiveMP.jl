...
@testitem "rules:Delta:unscented:in" begin
    ...
    @testset "Single input with known inverse" begin
        @test_rules [check_type_promotion = false] DeltaFn{g}(
            ...
            (
                input = (
                    ...
                    m_ins = nothing,
                    meta = DeltaMeta(; method = Unscented(), inverse = g_inv)
                ),
                output = NormalMeanVariance(
                    2.6255032138433307, 0.10796282966583703
                )
            ),
            (
                input = (
    ...
    @testset "Multiple input with known inverse" begin
        @test_rules [check_type_promotion = false] DeltaFn{h}(
            ...
            (
                input = (
                    ...
                    meta = DeltaMeta(;
                        method = Unscented(), inverse = (h_inv_x, h_inv_z)
                    )
                ),
                output = NormalMeanVariance(
                    2.6187538476660848, 0.14431487274498522
                )
            ),
            (
                input = (
        ...
        @test_rules [check_type_promotion = false] DeltaFn{h}(
            ...
            (
                ...
                output = NormalMeanVariance(
                    2.0000000002328306, 19.00000100088073
                )
            ),
            (
                input = (
