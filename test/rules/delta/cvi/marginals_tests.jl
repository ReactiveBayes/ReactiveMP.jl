        ...
        for enforce in (Val(false), Val(true)),
            ...
            @test_marginalrules [check_type_promotion = false, atol = 1e-2] DeltaFn{
                ...
                (
                    ...
                    output = FactorizedJoint((NormalMeanVariance(0.5, 0.5),))
                ),
                (
                    input = (
                    ...
                    output = FactorizedJoint((NormalMeanVariance(1.0, 0.5),))
                ),
                (
                    input = (
                        m_out = NormalMeanVariance(10.0, 1.0),
                        m_ins = ManyOf(NormalMeanVariance()),
                        meta = test_meta
                    ),
    ...
    @testset "id, x ~ MvNormal, y ~ MvNormal" begin
        for enforce in (Val(false), Val(true)),
            ...
            @test_marginalrules [check_type_promotion = false, atol = 1e-2] DeltaFn{
                ...
                (
                    input = (
                        m_out = MvGaussianMeanCovariance(ones(2)),
                ...
                (
                    input = (
                        m_out = MvGaussianMeanCovariance(ones(2) * 10),
                        m_ins = ManyOf(MvGaussianMeanCovariance(zeros(2))),
                        meta = test_meta
                    ),
                    output = FactorizedJoint((
                ...
                (
                    input = (
                        m_out = MvGaussianMeanCovariance(ones(2), [2 -1; -1 2]),
                        m_ins = ManyOf(MvGaussianMeanCovariance(zeros(2))),
                        meta = test_meta
                    ),
        ...
        for enforce in (Val(false), Val(true)),
            ...
            @test_marginalrules [check_type_promotion = false, atol = 1e-2] DeltaFn{
                ...
                (
                    ...
                    output = FactorizedJoint((NormalMeanVariance(0, 0.5),))
                ),
                (
                    input = (
                ...
                (
                    ...
                    output = FactorizedJoint((NormalMeanVariance(4.5, 0.5),))
                )
            ]
        end
    end
        ...
        for enforce in (Val(false), Val(true)),
            ...
            @test_marginalrules [check_type_promotion = false, atol = 1e-1] DeltaFn{
                ...
                (
                    ...
                    output = FactorizedJoint((Gamma(1, 1 / 2),))
                ),
                (
                    input = (
                        m_out = GammaShapeRate(1, 1),
                    ...
                    output = FactorizedJoint((Gamma(1, 1 / 3),))
                )
            ]
        end
        ...
        for enforce in (Val(false), Val(true)),
            ...
            @test_marginalrules [check_type_promotion = false, atol = 1e-1] DeltaFn{
                ...
                (
                    ...
                    output = FactorizedJoint((Gamma(2, 1 / 3),))
                ),
                (
                    input = (
                    ...
                    output = FactorizedJoint((Gamma(3, 1 / 5),))
                )
            ]
        end
    end
