...
@testitem "rules:Delta:cvi:in" begin
    @test_rules [check_type_promotion = true] DeltaFn{g}((:in, k = 1), Marginalisation) [
        ...
        (input = (q_ins = FactorizedJoint((GammaShapeRate(2, 2),)), m_in = GammaShapeRate(1, 1), meta = DeltaMeta(method = CVI(1, 1, EmptyOptimizer()))), output = Gamma(2.0, 1.0))
    ]
