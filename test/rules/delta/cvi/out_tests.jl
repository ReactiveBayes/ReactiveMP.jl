...
@testitem "rules:Delta:cvi:out" begin
    ...
    @testset "Exact value comparison (Pointmass)" begin
        for N in (500, 1000), i in 1:10
            test_meta = DeltaMeta(
                method = CVI(StableRNG(123), N, 1, EmptyOptimizer())
            )
            cvi_out_test(id, FactorizedJoint((PointMass(i),)), test_meta, i)
            cvi_out_test(
                id, FactorizedJoint((PointMass([i, i]),)), test_meta, [i, i]
            )
            cvi_out_test(
                first_argument,
                FactorizedJoint((PointMass(i), PointMass(i + 1))),
                test_meta,
                i
            )
            cvi_out_test(
                first_argument,
                FactorizedJoint((PointMass([i, i]), PointMass(i + 1))),
                test_meta,
                [i, i]
            )
            cvi_out_test(
    ...
    @testset "Multivariate normal distributions sampling" begin
        test_meta = DeltaMeta(
            method = CVI(StableRNG(123), 1000, 1, EmptyOptimizer())
        )
    ...
    @testset "Univariate Normal" begin
        ...
        for i in 1:5
            cvi_out_test(
                first_argument,
                FactorizedJoint((PointMass(i), NormalMeanVariance())),
                test_meta,
                i
            )
            cvi_out_test(
    ...
    @testset "Gamma shape rate" begin
        ...
        for i in 1:100
            cvi_out_test(
                first_argument,
                FactorizedJoint((PointMass(i), GammaShapeRate())),
                test_meta,
                i
            )
            cvi_out_test(
