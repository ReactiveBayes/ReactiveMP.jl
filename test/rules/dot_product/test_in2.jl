...
@testset "rules:typeof(dot):in2" begin
    @testset "Belief Propagation: (m_out::UnivariateNormalDistributionsFamily, m_in1::PointMass)" begin
        @test_rules [with_float_conversions = true] typeof(dot)(:in2, Marginalisation) [
            (
                input = (m_out = NormalMeanVariance(2.0, 2.0), m_in1 = PointMass(-1.0), meta = NoCorrection()),
                output = NormalWeightedMeanPrecision(-1.0, 0.5)
            ),
            (
                input = (m_out = NormalMeanPrecision(1.0, 1.0), m_in1 = PointMass(-2.0), meta = NoCorrection()),
                output = NormalWeightedMeanPrecision(-2.0, 4.0)
            ),
            (
                input = (m_out = NormalWeightedMeanPrecision(2.0, 1.0), m_in1 = PointMass(-1.0), meta = NoCorrection()),
        ...
        @test_rules [with_float_conversions = true] typeof(dot)(:in2, Marginalisation) [
            (
                input = (m_out = NormalMeanVariance(2.0, 2.0), m_in1 = PointMass(-1.0), meta = TinyCorrection()),
                output = NormalWeightedMeanPrecision(-1.0, 0.5)
            ),
            (
                input = (m_out = NormalMeanPrecision(1.0, 1.0), m_in1 = PointMass(-2.0), meta = TinyCorrection()),
                output = NormalWeightedMeanPrecision(-2.0, 4.0)
            ),
            (
                input = (
                    m_out = NormalWeightedMeanPrecision(2.0, 1.0),
                    m_in1 = PointMass(-1.0),
                    meta = TinyCorrection()
                ),
                output = NormalWeightedMeanPrecision(-2.0, 1.0)
        ...
        @test_rules [with_float_conversions = true] typeof(dot)(:in2, Marginalisation) [
            (
                input = (m_out = NormalMeanVariance(2.0, 2.0), m_in1 = PointMass(0.0), meta = NoCorrection()),
                output = NormalWeightedMeanPrecision(0.0, 0.0)
            ),
            (
                input = (m_out = NormalMeanPrecision(1.0, 1.0), m_in1 = PointMass(0.0), meta = NoCorrection()),
                output = NormalWeightedMeanPrecision(0.0, 0.0)
            ),
            (
                input = (m_out = NormalWeightedMeanPrecision(2.0, 1.0), m_in1 = PointMass(0.0), meta = NoCorrection()),
                output = NormalWeightedMeanPrecision(0.0, 0.0)
            )
        ]
        ...
        @test_rules [with_float_conversions = true] typeof(dot)(:in2, Marginalisation) [
            (
                input = (m_out = NormalMeanVariance(2.0, 2.0), m_in1 = PointMass(0.0), meta = TinyCorrection()),
                output = NormalWeightedMeanPrecision(0.0, tiny)
            ),
            (
                input = (m_out = NormalMeanPrecision(1.0, 1.0), m_in1 = PointMass(0.0), meta = TinyCorrection()),
                output = NormalWeightedMeanPrecision(0.0, tiny)
            ),
            (
                input = (
    ...
    @testset "Belief Propagation: (m_out::UnivariateNormalDistributionsFamily, m_in1::PointMass)" begin
        @test_rules [with_float_conversions = true] typeof(dot)(:in2, Marginalisation) [
            ...
            (
                input = (
                    m_out = NormalMeanPrecision(1.0, inv(2.0)),
