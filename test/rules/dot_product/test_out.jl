...
@testset "rules:typeof(dot):out" begin
    ...
    @testset "Belief Propagation: (m_in1::MultivariateNormalDistributionsFamily, m_in2::PointMass)" begin
        @test_rules [check_type_promotion = true] typeof(dot)(:out, Marginalisation) [
            (
                input = (m_in1 = MvNormalMeanCovariance([-1.0, 1.0], [2.0 -1.0; -1.0 4.0]), m_in2 = PointMass([4.0, 1.0]), meta = TinyCorrection()),
                output = NormalMeanVariance(-3, 28)
            ),
            (
                input = (m_in1 = MvNormalMeanPrecision([2.0, 1.0], [2.0 -0.5; -0.5 5.0]), m_in2 = PointMass([2.0, 2.0]), meta = TinyCorrection()),
                output = NormalMeanVariance(6.0, 128 / 39)
            ),
            (
                input = (m_in1 = MvNormalWeightedMeanPrecision([3.0, 2.0], [10.0 1.0; 1.0 20.0]), m_in2 = PointMass([-1.0, 3.0]), meta = TinyCorrection()),
    ...
    @testset "Belief Propagation: (m_in1::PointMass, m_in2::MultivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = true] typeof(dot)(:out, Marginalisation) [
            (
                input = (m_in1 = PointMass([4.0, 1.0]), m_in2 = MvNormalMeanCovariance([-1.0, 1.0], [2.0 -1.0; -1.0 4.0]), meta = TinyCorrection()),
                output = NormalMeanVariance(-3, 28)
            ),
            (
                input = (m_in1 = PointMass([2.0, 2.0]), m_in2 = MvNormalMeanPrecision([2.0, 1.0], [2.0 -0.5; -0.5 5.0]), meta = TinyCorrection()),
                output = NormalMeanVariance(6.0, 128 / 39)
            ),
            (
                input = (m_in1 = PointMass([-1.0, 3.0]), m_in2 = MvNormalWeightedMeanPrecision([3.0, 2.0], [10.0 1.0; 1.0 20.0]), meta = TinyCorrection()),
                output = NormalMeanVariance(-7 / 199, 116 / 199)
            )
        ]
