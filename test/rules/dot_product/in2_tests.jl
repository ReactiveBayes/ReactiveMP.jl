...
@testitem "rules:typeof(dot):in2" tags = [:rules] begin
    using ReactiveMP, BayesBase, Random, ExponentialFamily, Distributions

    import ReactiveMP: @test_rules
    import LinearAlgebra: dot
    import MatrixCorrectionTools:
        NoCorrection, AddToDiagonalEntries, ReplaceZeroDiagonalEntries

    @testset "Belief Propagation: (m_out::UnivariateNormalDistributionsFamily, m_in1::PointMass)" begin
        @test_rules [check_type_promotion = true] typeof(dot)(
            :in2, Marginalisation
        ) [
            (
                input = (
                    m_out = NormalMeanVariance(2.0, 2.0),
                    m_in1 = PointMass(-1.0),
                    meta = NoCorrection(),
                ),
    ...
    @testset "Belief Propagation: (m_out::UnivariateNormalDistributionsFamily, m_in1::PointMass)" begin
        ...
        @test_rules [check_type_promotion = true] typeof(dot)(
            ...
            (
                ...
                output = MvNormalWeightedMeanPrecision(
                    [-1.0, 1.0], [0.5+tiny -0.5; -0.5 0.5+tiny]
                ),
            ),
            (
                input = (
                ...
                output = MvNormalWeightedMeanPrecision(
                    [1.0, 0.5], [2.0+tiny 1.0; 1.0 0.5+tiny]
                ),
            ),
            (
                input = (
                ...
                output = MvNormalWeightedMeanPrecision(
                    [-1.0, 3.0], [1.0+tiny -3.0; -3.0 9.0+tiny]
                ),
            ),
        ]
    end
