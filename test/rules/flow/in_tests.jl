...
@testitem "rules:Flow:in" begin
    ...
    @testset "Belief Propagation: (m_out::MvNormalMeanCovariance, ) (Linearization)" begin
        @test_rules [check_type_promotion = false, atol = 1e-5] Flow(
            ...
            (
                input = (
                    m_out = MvNormalMeanCovariance([3.0, -1.5], diagm(ones(2))),
                    meta = meta
                ),
    ...
    @testset "Belief Propagation: (m_out::MvNormalMeanPrecision, ) (Linearization)" begin
        @test_rules [check_type_promotion = false, atol = 1e-5] Flow(
            ...
            (
                input = (
                    m_out = MvNormalMeanPrecision([3.0, -1.5], diagm(ones(2))),
                    meta = meta
                ),
            ...
            (
                input = (
                    m_out = MvNormalMeanPrecision([-5.0, -1.5], diagm(ones(2))),
                    meta = meta
                ),
