...
@testitem "rules:Flow:out" begin
    ...
    params = [1.0, 2.0, 3.0]
    model = FlowModel(
        2, (AdditiveCouplingLayer(PlanarFlow(); permute = false),)
    )
    ...
    @testset "Belief Propagation: (m_in::MvNormalMeanCovariance, ) (Linearization)" begin
        @test_rules [check_type_promotion = false, atol = 1e-5] Flow(
            ...
            (
                input = (
                    m_in = MvNormalMeanCovariance([3.0, -1.5], diagm(ones(2))),
                    meta = meta,
                ),
            ...
            (
                input = (
                    m_in = MvNormalMeanCovariance([-5.0, -1.5], diagm(ones(2))),
                    meta = meta,
                ),
            ...
            (
                input = (
                    m_in = MvNormalMeanCovariance(
                        [-5.0, -2.5], diagm([1.0, 2.0])
                    ),
                    meta = meta,
                ),
                output = MvNormalMeanCovariance(
                    [-5.0, -8.5], J2 * diagm([1.0, 2.0]) * J2'
                ),
            ),
        ]
    end
    ...
    @testset "Belief Propagation: (m_in::MvNormalMeanPrecision, ) (Linearization)" begin
        @test_rules [check_type_promotion = false, atol = 1e-5] Flow(
            ...
            (
                input = (
                    m_in = MvNormalMeanPrecision([3.0, -1.5], diagm(ones(2))),
                    meta = meta,
                ),
            ...
            (
                input = (
                    m_in = MvNormalMeanPrecision([-5.0, -1.5], diagm(ones(2))),
                    meta = meta,
                ),
            ...
            (
                input = (
                    m_in = MvNormalMeanPrecision(
                        [-5.0, -2.5], diagm([1.0, 0.5])
                    ),
                    meta = meta,
                ),
                output = MvNormalMeanPrecision(
                    [-5.0, -8.5], Ji2' * diagm([1.0, 0.5]) * Ji2
                ),
            ),
        ]
    end
