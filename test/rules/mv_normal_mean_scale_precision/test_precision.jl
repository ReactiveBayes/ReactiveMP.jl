...
@testset "rules:MvNormalMeanScalePrecision:precision" begin
    @testset "Variational: (q_out::MultivariateNormalDistributionsFamily, q_μ::MultivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = true, float32_atol = 1e-5] MvNormalMeanScalePrecision(:γ, Marginalisation) [
            (
    ...
    @testset "Variational: (q_out_μ::MultivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(:γ, Marginalisation) [
            (input = (q_out_μ = MvNormalMeanCovariance(ones(4), diageye(4)),), output = GammaShapeRate(2.0, 2.0)),
            (input = (q_out_μ = MvNormalMeanPrecision(ones(4), diageye(4)),), output = GammaShapeRate(2.0, 2.0)),
            (input = (q_out_μ = MvNormalWeightedMeanPrecision(ones(4), diageye(4)),), output = GammaShapeRate(2.0, 2.0))
        ]
        ...
        @test_rules [check_type_promotion = true, float32_atol = 1e-5] MvNormalMeanScalePrecision(:γ, Marginalisation) [
            (input = (q_out_μ = MvNormalMeanCovariance(a, A),), output = GammaShapeRate(2, 20)),
            (input = (q_out_μ = MvNormalMeanPrecision(a, A),), output = GammaShapeRate(2, 10.460349437749533)),
            (input = (q_out_μ = MvNormalWeightedMeanPrecision(a, A),), output = GammaShapeRate(2, 1.0832705501813922))
        ]
    end
end
