...
@testitem "rules:MvNormalMeanScalePrecision:mean" tags = [:rules, :alloc] begin
    ...
    @testset "Variational: (q_out::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(
            ...
            (
                ...
                output = MvNormalMeanScalePrecision([2.0, 1.0], 3.0),
            ),
        ]
    end
    ...
    @testset "Structured variational: (m_out::MvNormalMeanScalePrecision, q_γ::Gamma)" begin
        @test_rules [check_type_promotion = true] MvNormalMeanScalePrecision(
            ...
            (
                input = (
                    m_out = MvNormalMeanScalePrecision([2.0, 1.0], 3.0),
                    q_γ = Gamma(1.0, 1.0),
                ),
            ...
            (
                input = (
                    m_out = MvNormalMeanScalePrecision([0.0, 0.0], 4.0),
                    q_γ = GammaShapeRate(4.0, 2.0),
                ),
                output = MvNormalMeanScalePrecision(
            ...
            (
                input = (
                    m_out = MvNormalMeanScalePrecision([3.0, -1.0], 2.0),
                    q_γ = GammaShapeRate(2.0, 1.0),
                ),
                output = MvNormalMeanScalePrecision(
    ...
    @testset "Performance: MvNormalMeanScalePrecision rule allocates less than general rule" begin
        ...
        for n in (10, 100)
            ...
            m_out_general = MvNormalMeanCovariance(
