...
@testitem "rules:MvNormalMeanScaleMatrixPrecision:matrix" begin
    ...
    @testset "Variational: (q_out::MultivariateNormalDistributionsFamily, q_μ::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        ...
        @test_rules [check_type_promotion = true] MvNormalMeanScaleMatrixPrecision(:G, Marginalisation) [
            (
                input = (
                    q_out = MvNormalMeanCovariance([1.0; 2.0], [1.0 2.0; 2.0 3.0]), q_μ = MvNormalMeanCovariance([1.0; 2.0], [3.0 2.0; 2.0 4.0]), q_γ = GammaShapeRate(1.0, 1.0)
                ),
                output = Wishart(4, [0.5833333333333333 -0.3333333333333333; -0.3333333333333333 0.3333333333333333])
            ),
            (
                input = (
                    q_out = MvNormalMeanCovariance([2.0; 5.0], [3.0 2.0; 2.0 4.0]), q_μ = MvNormalMeanCovariance([3.0; 5.0], [1.0 2.0; 2.0 3.0]), q_γ = GammaShapeRate(2.0, 4.0)
                ),
    ...
    @testset "Structured variational: (q_out_μ::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        ...
        @test_rules [check_type_promotion = true] MvNormalMeanScaleMatrixPrecision(:G, Marginalisation) [
            (input = (q_out_μ = MvNormalWeightedMeanPrecision(ones(4), diageye(4)), q_γ = GammaShapeRate(1.0, 1.0)), output = Wishart(4, [0.5 0.0; 0.0 0.5])),
            (input = (q_out_μ = MvNormalWeightedMeanPrecision(2*ones(4), diageye(4)), q_γ = GammaShapeRate(2.0, 4.0)), output = Wishart(4, [1.0 0.0; 0.0 1.0])),
            (
                input = (q_out_μ = MvNormalWeightedMeanPrecision(3*ones(4), diageye(4)), q_γ = GammaShapeRate(22.0, 14.0)),
