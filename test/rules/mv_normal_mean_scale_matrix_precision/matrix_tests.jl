...
@testitem "rules:MvNormalMeanScaleMatrixPrecision:matrix" tags = [:rules] begin
        ...
        @test_rules [check_type_promotion = true] MvNormalMeanScaleMatrixPrecision(
            ...
            (
                ...
                output = Wishart(
                    4,
                    [
                        0.5833333333333333 -0.3333333333333333;
            ...
            (
                input = (
                    q_out = MvNormalMeanCovariance(
                        [2.0; 5.0], [3.0 2.0; 2.0 4.0]
                    ),
                    q_μ = MvNormalMeanCovariance(
                        [3.0; 5.0], [1.0 2.0; 2.0 3.0]
                    ),
                    q_γ = GammaShapeRate(2.0, 4.0),
                ),
                output = Wishart(
                    4,
                    [
                        0.736842105263158 -0.42105263157894735;
                        -0.42105263157894735 0.5263157894736842
                    ],
                ),
            ),
        ]
    ...
    @testset "Structured variational: (q_out_μ::MultivariateNormalDistributionsFamily, q_γ::Gamma)" begin
        ...
        @test_rules [check_type_promotion = true] MvNormalMeanScaleMatrixPrecision(
            ...
            (
                ...
                output = Wishart(
                    4, [0.3181818181818182 0.0; 0.0 0.3181818181818182]
                ),
            ),
        ]
    end
