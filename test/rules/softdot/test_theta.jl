...
@testset "rules:SoftDot:θ" begin
    ...
    @testset "VMP: Mean-field" begin
        ...
        @testset "(q_y::PointMass, q_x::MvNormalMeanCovariance, q_γ::PointMass)" begin
            @test_rules [with_float_conversions = true] SoftDot(:θ, Marginalisation) [
                (input = (q_y = PointMass(3.0), q_x = MvNormalMeanCovariance([5.0, 9.0], [11.0 13.0; 17.0 19.0]), q_γ = PointMass(2.0)), output = MvNormalWeightedMeanPrecision([30.0, 54.0], [72.0 116.0; 124.0 200.0])),
            ]
        end
        ...
        @testset "(q_y::PointMass, q_x::MvNormalMeanCovariance, q_γ::GammaShapeScale)" begin
            @test_rules [with_float_conversions = true] SoftDot(:θ, Marginalisation) [
                (input = (q_y = PointMass(3.0), q_x = MvNormalMeanCovariance([5.0, 9.0], [11.0 13.0; 17.0 19.0]), q_γ = GammaShapeScale(7.0, 23.0)), output = MvNormalWeightedMeanPrecision([2415.0, 4347.0], [5796.0 9338.0; 9982.0 16100.0])),
            ]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_x::NormalMeanVariance, q_γ::GammaShapeScale)" begin
            @test_rules [with_float_conversions = true] SoftDot(:θ, Marginalisation) [
                (input = (q_y = NormalMeanVariance(3.0, 7.0), q_x = NormalMeanVariance(2.0, 7.0), q_γ = GammaShapeScale(5.0, 11.0)), output = NormalWeightedMeanPrecision(330.0, 605.0)),
            ]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_x::MvNormalMeanCovariance, q_γ::PointMass)" begin
            @test_rules [with_float_conversions = true] SoftDot(:θ, Marginalisation) [
                (input = (q_y = NormalMeanVariance(3.0, 7.0), q_x = MvNormalMeanCovariance([5.0, 9.0], [11.0 13.0; 17.0 19.0]), q_γ = PointMass(2.0)), output = MvNormalWeightedMeanPrecision([30.0, 54.0], [72.0 116.0; 124.0 200.0])),
            ]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_x::MvNormalMeanCovariance, q_γ::GammaShapeScale)" begin
            @test_rules [with_float_conversions = true] SoftDot(:θ, Marginalisation) [
                (input = (q_y = NormalMeanVariance(3.0, 7.0), q_x = MvNormalMeanCovariance([5.0, 9.0], [11.0 13.0; 17.0 19.0]), q_γ = GammaShapeScale(7.0, 23.0)), output = MvNormalWeightedMeanPrecision([2415.0, 4347.0], [5796.0 9338.0; 9982.0 16100.0])),
            ]
        end
