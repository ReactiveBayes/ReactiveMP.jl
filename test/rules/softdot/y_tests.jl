    ...
    @testset "VMP: Mean-field" begin
        ...
        @testset "(q_θ::PointMass, q_x::PointMass, q_γ::PointMass" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = NormalMeanPrecision(33.0, 7.0)
            )]
        end
            ...
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                (
                    input = (
                        q_θ = PointMass(3.0),
                        q_x = PointMass(11.0),
                        q_γ = GammaShapeRate(7.0, 5.0)
                    ),
                    output = NormalMeanPrecision(33.0, 1.4)
                ),
                (
                    input = (
                        q_θ = PointMass(3.0),
                        q_x = PointMass(11.0),
                        q_γ = GammaShapeScale(7.0, 5.0)
                    ),
                    output = NormalMeanPrecision(33.0, 35.0)
                )
            ]
        end
        ...
        @testset "(q_θ::NormalMeanVariance, q_x::NormalMeanVariance, q_γ::{<:GammaDistributionsFamily})" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_θ = NormalMeanVariance(3.0, 17.0),
                    q_x = NormalMeanVariance(7.0, 11.0),
                    q_γ = PointMass(13.0)
                ),
                output = NormalMeanPrecision(21.0, 13.0)
            )]
        end
        ...
        @testset "(q_θ::NormalMeanVariance, q_x::NormalMeanVariance, q_γ::{<:GammaDistributionsFamily})" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                (
                    ...
                    output = NormalMeanPrecision(21.0, 2.6)
                ),
                (
                    input = (
                        q_θ = NormalMeanVariance(3.0, 17.0),
                    ...
                    output = NormalMeanPrecision(21.0, 65.0)
                )
            ]
        end
        ...
        @testset "(q_θ::MvNormalMeanCovariance, q_x::MvNormalMeanCovariance, q_γ::{<:GammaDistributionsFamily})" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    ...
                    q_x = MvNormalMeanCovariance([17.0, 19.0], [23.0, 29.0]),
                    q_γ = PointMass(31.0)
                ),
    ...
    @testset "VMP: structured rules" begin
        @testset "(q_θ::NormalMeanVariance, m_x::NormalMeanVariance, q_γ::Any" begin
            ...
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                (
                    input = (
                        m_x = NormalMeanVariance(1.0, 1.0),
                        q_θ = NormalMeanVariance(1.0, 1.0),
                ...
                (
                    input = (
                        m_x = NormalWeightedMeanPrecision(1.0, 1.0),
                        q_θ = NormalMeanPrecision(1.0, 2.0),
                        q_γ = GammaShapeScale(2.0, 1.0)
                    ),
                ...
                (
                    input = (
                        m_x = MvNormalMeanCovariance(
                            ones(order), diageye(order)
                        ),
                        q_θ = MvNormalMeanCovariance(
                            zeros(order), diageye(order)
                        ),
