    ...
    @testset "VMP: Mean-field" begin
        ...
        @testset "(q_y::PointMass, q_θ::PointMass, q_γ::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = NormalWeightedMeanPrecision(30.0, 50.0)
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::PointMass, q_γ::GammaShapeScale)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = PointMass(3.0),
                    q_θ = PointMass(5.0),
                    q_γ = GammaShapeScale(2.0, 7.0)
                ),
                output = NormalWeightedMeanPrecision(210.0, 350.0)
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::NormalMeanVariance, q_γ::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = NormalWeightedMeanPrecision(12.0, 22.0)
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::MvNormalMeanCovariance, q_γ::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = MvNormalWeightedMeanPrecision(
                    [30.0, 54.0], [72.0 116.0; 124.0 200.0]
                )
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::MvNormalMeanCovariance, q_γ::GammaShapeScale)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    ...
                    q_γ = GammaShapeScale(7.0, 23.0)
                ),
                output = MvNormalWeightedMeanPrecision(
                    [2415.0, 4347.0], [5796.0 9338.0; 9982.0 16100.0]
                )
            )]
        end
    ...
    @testset "VMP: structured rules" begin
        @testset "(m_y::NormalMeanVariance, q_θ::NormalMeanVariance, q_γ::Any)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                (
                    input = (
                        m_y = NormalWeightedMeanPrecision(1.0, 1.0),
        ...
        @testset "(m_y::UnivariateNormalDistributionsFamily, q_θ::MultivariateNormalDistributionsFamily, q_γ::Any)" begin
            order = 2
            @test_rules [check_type_promotion = false] SoftDot(
                :x, Marginalisation
            ) [
                (
                    input = (
                        m_y = NormalMeanVariance(0.0, 1.0),
                        q_θ = MvNormalMeanCovariance(
