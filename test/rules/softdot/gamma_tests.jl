...
@testitem "rules:SoftDot:γ" begin
    ...
    @testset "VMP: Mean-field" begin
        ...
        @testset "(q_y::PointMass, q_θ::PointMass, q_x::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                :γ, Marginalisation
            ) [(
                input = (
                    q_y = PointMass(3.0),
                ...
                output = GammaShapeRate(3 / 2, 49 / 2),
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::PointMass, q_x::NormalMeanVariance)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = PointMass(3.0),
                    q_θ = PointMass(5.0),
                    q_x = NormalMeanVariance(2.0, 7.0),
                ),
                output = GammaShapeRate(3 / 2, 224 / 2),
            )]
        end
        ...
        @testset "(q_y::PointMass, q_θ::NormalMeanVariance, q_x::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = PointMass(3.0),
                    q_θ = NormalMeanVariance(2.0, 7.0),
                    q_x = PointMass(5.0),
                ),
                output = GammaShapeRate(3 / 2, 224 / 2),
            )]
        ...
        @testset "(q_y::PointMass, q_θ::NormalMeanVariance, q_x::NormalMeanVariance)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = PointMass(3.0),
                    q_θ = NormalMeanVariance(2.0, 7.0),
                    q_x = NormalMeanVariance(5.0, 11.0),
                ),
                output = GammaShapeRate(3 / 2, 345 / 2),
            )]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_θ::PointMass, q_x::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = GammaShapeRate(3 / 2, 1092 / 2),
            )]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_θ::PointMass, q_x::NormalMeanVariance)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                output = GammaShapeRate(3 / 2, 92 / 2),
            )]
        end
        ...
        @testset "(q_y::NormalMeanVariance, q_θ::NormalMeanVariance, q_x::PointMass)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = NormalMeanVariance(3.0, 7.0),
                    q_θ = NormalMeanVariance(5.0, 9.0),
                    q_x = PointMass(2.0),
                ),
                output = GammaShapeRate(3 / 2, 92 / 2),
            )]
        ...
        @testset "(q_y::NormalMeanVariance, q_θ::NormalMeanVariance, q_x::NormalMeanVariance)" begin
            @test_rules [check_type_promotion = true] SoftDot(
                ...
                input = (
                    q_y = NormalMeanVariance(3.0, 7.0),
                    q_θ = NormalMeanVariance(5.0, 9.0),
                    q_x = NormalMeanVariance(11.0, 13.0),
                ),
                output = GammaShapeRate(3 / 2, 4242 / 2),
            )]
        end
    ...
    @testset "VMP: Incorrect Inputs" begin
        ...
        @testset "(q_y::MvNormalMeanCovariance, q_θ::CORRECT, q_x::CORRECT)" begin
            @test_throws DimensionMismatch @call_rule SoftDot(
                :γ, Marginalisation
            ) (
    ...
    @testset "Structured: (q_y_x::MultivariateNormalDistributionsFamily, q_γ::Any)" begin
        @test_rules [check_type_promotion = true] SoftDot(:γ, Marginalisation) [
            ...
            (
                input = (
                    q_y_x = MvNormalMeanCovariance(2 * ones(2), diageye(2)),
                    q_θ = NormalMeanPrecision(2.0, 1.0),
                ),
                output = GammaShapeRate(3 / 2, 7.0),
            ),
        ]
    end
        ...
        @test_rules [check_type_promotion = true] SoftDot(:γ, Marginalisation) [
            (
                input = (
                    q_y_x = MvNormalMeanCovariance(
                        ones(order + 1), diageye(order + 1)
                    ),
                    q_θ = MvNormalMeanPrecision(ones(order), diageye(order)),
                ),
            ...
            (
                input = (
                    ...
                    q_θ = MvNormalMeanPrecision(zeros(order), diageye(order)),
                ),
