...
@testset "rules:NormalMeanVariance:var" begin
    ...
    @testset "Variational: (q_out::Any, q_μ::Any)" begin
        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            (
                input = (q_out = PointMass(-1.0), q_μ = PointMass(2.0)),
                output = GammaInverse(-0.5, 4.5, check_args = false)
            ),
            (
                input = (q_out = PointMass(1.0), q_μ = PointMass(2.0)),
                output = GammaInverse(-0.5, 0.5, check_args = false)
            ),
            (
                input = (q_out = PointMass(2.0), q_μ = PointMass(1.0)),
                output = GammaInverse(-0.5, 0.5, check_args = false)
            )
        ]
        ...
        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            (
                input = (q_out = NormalMeanVariance(-1.0, 2.0), q_μ = PointMass(2.0)),
                output = GammaInverse(-0.5, 5.5, check_args = false)
            ),
            (
                input = (q_out = NormalMeanPrecision(1.0, 4.0), q_μ = PointMass(3.0)),
                output = GammaInverse(-0.5, 2.125, check_args = false)
            ),
            (
                input = (q_out = NormalWeightedMeanPrecision(2.0, 4.0), q_μ = PointMass(1.0)),
                output = GammaInverse(-0.5, 0.25, check_args = false)
            )
        ]

        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            (
                input = (q_out = PointMass(2.0), q_μ = NormalMeanVariance(-1.0, 2.0)),
                output = GammaInverse(-0.5, 5.5, check_args = false)
            ),
            (
                input = (q_out = PointMass(3.0), q_μ = NormalMeanPrecision(1.0, 4.0)),
                output = GammaInverse(-0.5, 2.125, check_args = false)
            ),
            (
                input = (q_out = PointMass(1.0), q_μ = NormalWeightedMeanPrecision(2.0, 4.0)),
                output = GammaInverse(-0.5, 0.25, check_args = false)
            )
        ]
    ...
    @testset "Variational: (q_out_μ::Any)" begin
        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            (
                input = (q_out_μ = MvNormalMeanCovariance([1.0, 1.0], [1.0 0.0; 0.0 1.0]),),
                output = GammaInverse(-0.5, 1.0, check_args = false)
            ),
            (
                input = (q_out_μ = MvNormalMeanCovariance([2.0, 3.0], [2.0 -0.1; -0.1 3.0]),),
                output = GammaInverse(-0.5, 3.1, check_args = false)
            ),
            (
                input = (q_out_μ = MvNormalMeanCovariance([4.0, 1.0], [4.0 1.0; 1.0 9.0]),),
                output = GammaInverse(-0.5, 10.0, check_args = false)
            )
        ]

        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            (
                input = (q_out_μ = MvNormalMeanPrecision([1.0, 1.0], [1.0 0.0; 0.0 1.0]),),
                output = GammaInverse(-0.5, 1.0, check_args = false)
            ),
            (
                input = (q_out_μ = MvNormalMeanPrecision([2.0, 3.0], [2.0 -0.1; -0.1 3.0]),),
                output = GammaInverse(-0.5, 1079.0 / 1198.0, check_args = false)
            ),
            (
                input = (q_out_μ = MvNormalMeanPrecision([4.0, 1.0], [4.0 1.0; 1.0 9.0]),),
                output = GammaInverse(-0.5, 33.0 / 7.0, check_args = false)
            )
        ]
        ...
        @test_rules [with_float_conversions = true] NormalMeanVariance(:v, Marginalisation) [
            ...
            (
                ...
                output = GammaInverse(-0.5, 5.0 / 7.0, check_args = false)
            )
        ]
    end
end
