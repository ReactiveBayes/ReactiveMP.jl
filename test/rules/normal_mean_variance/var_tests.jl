...
@testitem "rules:NormalMeanVariance:var" begin
    ...
    @testset "Belief Propagation: (m_out::PointMass, m_μ::UnivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = false] NormalMeanVariance(
            :v, Marginalisation
        ) [
            (
                input = (
    ...
    @testset "Belief Propagation: (m_out::UnivariateNormalDistributionsFamily, m_μ::UnivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = false] NormalMeanVariance(
            ...
            (
                input = (
                    m_out = NormalMeanVariance(0.0, 1.0),
                    m_μ = NormalMeanVariance(-2.0, 1.0)
                ),
                output = ContinuousUnivariateLogPdf(
                    DomainSets.HalfLine(),
            ...
            (
                input = (
                    m_out = NormalMeanVariance(1.0, 0.5),
                    m_μ = NormalMeanVariance(0.0, 2.0)
                ),
                output = ContinuousUnivariateLogPdf(
                    DomainSets.HalfLine(),
            ...
            (
                input = (
                    m_out = NormalMeanVariance(0.5, 2.0),
                    m_μ = NormalMeanVariance(1.5, 0.5)
                ),
                output = ContinuousUnivariateLogPdf(
                    DomainSets.HalfLine(),
                    (x) -> -log(2.5 + x) / 2 - 1 / (2 * (2.5 + x))
    ...
    @testset "Variational: (q_out::Any, q_μ::Any)" begin
        ...
        @test_rules [check_type_promotion = false] NormalMeanVariance(
            ...
            (
                input = (
                    q_out = PointMass(1.0),
                    q_μ = NormalWeightedMeanPrecision(2.0, 4.0)
                ),
                output = GammaInverse(-0.5, 0.25; check_args = false)
            )
        ]
        ...
        @test_rules [check_type_promotion = false] NormalMeanVariance(
            ...
            (
                input = (
                    q_out = NormalMeanVariance(2.0, 4.0),
                    q_μ = NormalMeanVariance(-1.0, 2.0)
                ),
                output = GammaInverse(-0.5, 7.5; check_args = false)
            ),
            (
                input = (
                    q_out = NormalMeanPrecision(3.0, 3.0),
                    q_μ = NormalMeanPrecision(1.0, 4.0)
                ),
                output = GammaInverse(-0.5, 55.0 / 24.0; check_args = false)
            ),
            (
                input = (
                    q_out = NormalWeightedMeanPrecision(1.0, 1.0),
                    q_μ = NormalWeightedMeanPrecision(2.0, 4.0)
                ),
    ...
    @testset "Variational: (q_out_μ::Any)" begin
        ...
        @test_rules [check_type_promotion = false] NormalMeanVariance(
            ...
            (
                ...
                output = GammaInverse(
                    -0.5, 1079.0 / 1198.0; check_args = false
                )
            ),
            (
                input = (
