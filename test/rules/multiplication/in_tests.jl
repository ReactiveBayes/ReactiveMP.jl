...
@testitem "rule:typeof(*):in" begin
    ...
    @testset "Belief Propagation: (m_A::PointMass{<:Real}, m_out::MultivariateNormalDistributionsFamily)" begin
        @test_rules [check_type_promotion = true] (*)(:in, Marginalisation) [
            (
                input = (
                    m_A = PointMass(2),
                    m_out = MvNormalMeanCovariance([2, 4], [12 8; 8 24]),
                ),
                output = MvNormalMeanCovariance([1, 2], [3 2; 2 6]),
            ),
            (
                input = (
                    m_A = PointMass(0.5),
                    m_out = MvNormalMeanPrecision([1, 2], [12 8; 8 24]),
                ),
                output = MvNormalMeanPrecision([2, 4], [3 2; 2 6]),
            ),
            (
                input = (
                    m_A = PointMass(0.5),
                    m_out = MvNormalWeightedMeanPrecision([2, 4], [12 8; 8 24]),
                ),
                output = MvNormalWeightedMeanPrecision([1, 2], [3 2; 2 6]),
            ),
        ]
    end
    ...
    @testset "Univariate Gaussian messages" begin
        ...
        OutMessage_1 = @call_rule typeof(*)(:in, Marginalisation) (
            m_out = d1, m_A = d2
        )
        OutMessage_2 = @call_rule typeof(*)(:in, Marginalisation) (
            m_out = d1, m_A = d3
        )
        OutMessage_3 = @call_rule typeof(*)(:in, Marginalisation) (
            m_out = d2, m_A = d3
        )
    ...
    @testset "messages of type Any" begin
        ...
        OutMessage = @call_rule typeof(*)(:in, Marginalisation) (
            m_out = d1, m_A = d2
        )
