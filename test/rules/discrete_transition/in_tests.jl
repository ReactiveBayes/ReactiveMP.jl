    ...
    @test_rules [check_type_promotion = false] DiscreteTransition(
        ...
        (
            input = (
                ...
                q_a = DirichletCollection(diageye(3) .+ tiny),
            ),
            output = Categorical([0.0, 1.0, 0.0]),
        ),
    ]
end
...
@testitem "rules:DiscreteTransition:in:Variational Bayes: (q_out::Any, q_a::PointMass)" tags = [
    ...
    @test_rules [check_type_promotion = false] DiscreteTransition(
        ...
        (
            input = (
                q_out = PointMass([0.1, 0.4, 0.5]),
                q_a = PointMass([0.2 0.4 0.1; 0.1 0.3 0.6; 0.7 0.3 0.3]),
            ),
            output = Categorical(
                [
...
@testitem "rules:DiscreteTransition:in:Variational Bayes: (m_out::Categorical, q_a::DirichletCollection)" tags = [
    ...
    @test_rules [check_type_promotion = false] DiscreteTransition(
        ...
        (
            input = (
                m_out = Categorical([0.3, 0.3, 0.4]),
                q_a = DirichletCollection(diageye(3) .+ 1),
            ),
            output = Categorical(
                [
...
@testitem "rules:DiscreteTransition:in:Belief Propagation: (m_out::Categorical, q_a::PointMass)" tags = [
    ...
    @test_rules [check_type_promotion = false] DiscreteTransition(
        ...
        (
            input = (
                m_out = Categorical([1.0, 0.0, 0.0]),
                q_a = PointMass([0.0 0.0 0.0; 0.5 0.5 0.5; 0.5 0.5 0.5]),
            ),
