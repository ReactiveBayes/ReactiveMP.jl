@testitem "rules:MultinomialPolya:psi" begin
    ...
    @testset "Expectation Propagation: (q_x::PointMass, q_N::PointMass, m_ψ::GaussianDistributionsFamily, meta::Union{Nothing, MultinomialPolyaMeta})" begin
        q_x = PointMass([0.2, 0.3, 0.5])
        q_N = PointMass(1.0)
        m_ψ = MvNormalWeightedMeanPrecision(zeros(2), diageye(2))
        metas = [nothing, MultinomialPolyaMeta(5, MersenneTwister(10)), MultinomialPolyaMeta(10, MersenneTwister(42))]

        # Expected values based on the rule calculations
        # η = η_ψ + (x[1:K-1] - Nks/2)
        # Λ = Λ_ψ + Diagonal(ω)
        η_expected = [0.2 - (1 - 0)/2, 0.3 - (1 - 0.2)/2]  # Nks = [1, 0.8]
        Λ_expected = diageye(2) + Diagonal([0.25, 0.2])    # ω = Nks/4 when ψ=0
        ...
        for meta in metas
            ...
            @test weightedmean(out) ≈ η_expected rtol = 1e-1  # Allow some tolerance for sampling
            @test diag(precision(out)) ≈ diag(Λ_expected) atol = 0.1
        end
    end
end
