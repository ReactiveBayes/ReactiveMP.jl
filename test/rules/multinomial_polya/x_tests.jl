@testitem "rules:MultinomialPolya:x" begin
    ...
    @testset "Predictive Distribution: (q_N::PointMass, q_ψ::GaussianDistributionsFamily, meta::Union{Nothing, MultinomialPolyaMeta})" begin
        ...
        @test_rules [check_type_promotion = false] MultinomialPolya(:x, Marginalisation) [(input = (q_N = q_N, q_ψ = q_ψ, meta = metas[1]), output = dist_expected)]
    ...
    @testset "Predictive Distribution: (q_N::Poisson, q_ψ::GaussianDistributionsFamily, meta::Union{Nothing, MultinomialPolyaMeta})" begin
