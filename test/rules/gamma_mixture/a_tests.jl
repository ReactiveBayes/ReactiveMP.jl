@testitem "rules:GammaMixture:a" begin
    ...
    @testset "Variational : (:a, k=1)" begin
        ...
        @test_rules [check_type_promotion = false] GammaMixture{2}((:a, k = 1), Marginalisation) [
            (input = (q_out = GammaShapeRate(2.0, 1.0), q_switch = Categorical([0.7, 0.3]), q_b = GammaShapeRate(1.0, 2.0)), output = GammaShapeLikelihood(p1, γ1)),
            (input = (q_out = GammaShapeRate(3.0, 2.0), q_switch = Bernoulli(0.5), q_b = GammaShapeRate(1.0, 3.0)), output = GammaShapeLikelihood(p2, γ2))
        ]
    end
end
