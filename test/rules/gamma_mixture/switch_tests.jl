@testitem "rules:GammaMixture:out" tags = [:rules] begin
    ...
    @testset "Variational : (:switch)" begin
        @test_rules [check_type_promotion = false, atol = 1.0e-6] GammaMixture{2}(
            ...
            (
                input = (
                    ...
                    q_a = ManyOf(
                        (
                            GammaShapeRate(1.0, 2.0), GammaShapeRate(2.0, 3.0),
                        )
                    ),
                    q_b = ManyOf(
                        (
                            GammaShapeRate(3.0, 1.0), GammaShapeRate(4.0, 2.0),
                        )
                    ),
                ),
