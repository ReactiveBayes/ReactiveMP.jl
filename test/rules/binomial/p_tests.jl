@testitem "rules:Binomial:p" begin
    ...
    @testset "Variational Message Passing: (q_k::PointMass, q_n::PointMass)" begin
        @test_rules [check_type_promotion = false] Binomial(:p, Marginalisation) [
            (input = (q_n = PointMass(10), q_k = PointMass(5)), output = Beta(6, 6)), (input = (q_n = PointMass(5), q_k = PointMass(3)), output = Beta(4, 3))
        ]
    end

    @testset "Variational Message Passing: (q_k::Binomial, q_n::PointMass)" begin
        @test_rules [check_type_promotion = false] Binomial(:p, Marginalisation) [
            (input = ( q_n = PointMass(10), q_k = Binomial(5, 0.1)), output = Beta(mean(Binomial(5, 0.1)) + 1, 10 - mean(Binomial(5, 0.1)) + 1)),
            (input = ( q_n = PointMass(5), q_k = Binomial(3, 0.4)), output = Beta(mean(Binomial(3, 0.4)) + 1, 5 - mean(Binomial(3, 0.4)) + 1))
        ]
    end
    ...
    @testset "Sum Product Message Passing: (m_k::Binomial, m_n::PointMass)" begin
        @test_rules [check_type_promotion = false] Binomial(:p, Marginalisation) [
            (input = (m_n = PointMass(10), m_k = PointMass(5)), output = Beta(5 + 1, 10 - 5 + 1)),
            (input = (q_n = PointMass(5), q_k = Binomial(3, 0.4)), output = Beta(mean(Binomial(3, 0.4)) + 1, 5 - mean(Binomial(3, 0.4)) + 1))
        ]
    end
    @testset "SumProduct Message Passing: (m_k::Binomia, m_n::Binomial)" begin
        ...
        grid = collect(0:dt:1)
        for k in ks, n in ns
            output = @call_rule Binomial(:p, Marginalisation) (m_n = n, m_k = k)
            ...
            mean_sp_message = mean(map(x -> pf(x) * x, grid))
            @test -Inf < entropy_sp_message < Inf
            @test !isnan(entropy_sp_message)
            @test 0 ≤ mean_sp_message ≤ 1
        end
    end
end
