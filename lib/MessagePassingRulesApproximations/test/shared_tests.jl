@testitem "Shared approximation methods" tags = [:approximations] begin
    using MessagePassingRulesApproximations

    @testset "__starts_at tests" begin
        import MessagePassingRulesApproximations: __starts_at
    ...
    @testset "__as_vec tests" begin
        import MessagePassingRulesApproximations: __as_vec
        ...
        @static if VERSION >= v"1.7"
            ...
            @test @inferred(__as_vec(([3.0, 2.0], [2.0], 1))) ==
                [3.0, 2.0, 2.0, 1.0]
            @test @inferred(__as_vec((3, 2, [1.0 0.0; 0.0 1.0]))) ==
                [3.0, 2.0, 1.0, 0.0, 0.0, 1.0]
            @test @inferred(__as_vec(([3.0, 2.0], 2, [1.0 0.0; 0.0 1.0]))) ==
                [3.0, 2.0, 2.0, 1.0, 0.0, 0.0, 1.0]
        else
            ...
            @test __as_vec((3, 2, [1.0 0.0; 0.0 1.0])) ==
                [3.0, 2.0, 1.0, 0.0, 0.0, 1.0]
            @test __as_vec(([3.0, 2.0], 2, [1.0 0.0; 0.0 1.0])) ==
                [3.0, 2.0, 2.0, 1.0, 0.0, 0.0, 1.0]
        end
    end

    @testset "__splitjoin tests" begin
        import MessagePassingRulesApproximations: __splitjoin

        # `@inferred` is need for type-stability
        @test @inferred(__splitjoin([1, 2, 3], ((), (), ()))) == (1, 2, 3)
        @test @inferred(__splitjoin([1, 2, 3], ((), (1,), ()))) == (1, [2], 3)
        @test @inferred(__splitjoin([1, 2, 3], ((1,), (1,), ()))) ==
            ([1], [2], 3)
        @test @inferred(__splitjoin([1, 2, 3], ((1,), (1,), (1,)))) ==
            ([1], [2], [3])
        @test @inferred(
            __splitjoin([1, 2, 1, 0, 0, 1], ((1,), (1,), (2, 2)))
        ) == ([1], [2], [1 0; 0 1])
        @test @inferred(
            __splitjoin([1, 0, 0, 1, 2, 1, 0, 0, 1], ((2, 2), (1,), (2, 2)))
        ) == ([1 0; 0 1], [2], [1 0; 0 1])
        @test @inferred(
            __splitjoin([1, 0, 0, 1, 2, 1, 0, 0, 1], ((2, 2), (), (2, 2)))
        ) == ([1 0; 0 1], 2, [1 0; 0 1])
    end
end
