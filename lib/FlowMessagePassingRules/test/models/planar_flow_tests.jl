# From v6's `test/nodes/predefined/flow/coupling_flows/planar_flow_tests.jl`.

@testitem "models:Planar Flow" tags = [:models] begin
    using FlowMessagePassingRules, StableRNGs
    using FlowMessagePassingRules: getdim, getu, getb, getw, getall, setu!, setb!, setw!
    ...
    @testset "Constructors" begin
        ...
        f = PlanarFlow()
        @test typeof(f) == FlowMessagePassingRules.PlanarFlowPlaceholder

        # check when creating placeholder
        f = FlowMessagePassingRules.PlanarFlowPlaceholder()
        @test typeof(f) == FlowMessagePassingRules.PlanarFlowPlaceholder

        # check when creating empty planar flow
        f = FlowMessagePassingRules.PlanarFlowEmpty(3)
        @test typeof(f) == FlowMessagePassingRules.PlanarFlowEmpty{3}
        @test getdim(f) == 3
    ...
    @testset "Prepare-Compile" begin
        f = FlowMessagePassingRules.PlanarFlowPlaceholder()
        @test FlowMessagePassingRules.prepare(2, f) == FlowMessagePassingRules.PlanarFlowEmpty(2)
        @test FlowMessagePassingRules.prepare(5, f) == FlowMessagePassingRules.PlanarFlowEmpty(5)
        @test FlowMessagePassingRules.prepare(9, f) == FlowMessagePassingRules.PlanarFlowEmpty(9)

        f = FlowMessagePassingRules.PlanarFlowEmpty(1)
        params = [1.0, 2.0, 3.0]
        fc = FlowMessagePassingRules.compile(rng, f)
        fcp = FlowMessagePassingRules.compile(f, params)
        ...
        f = FlowMessagePassingRules.PlanarFlowEmpty(2)
    ...
    @testset "nr_params" begin
        for k in 1:10
            f = FlowMessagePassingRules.PlanarFlowEmpty(k)
            fc = compile(rng, f)
    ...
    @testset "Get-Set" begin
        ...
        # check getdim
        f = FlowMessagePassingRules.PlanarFlowEmpty(2)
        @test FlowMessagePassingRules.getdim(f) == 2
