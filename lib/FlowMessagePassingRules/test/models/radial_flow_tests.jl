@testitem "models:Radial Flow" tags = [:models] begin
    ...
    @testset "Constructor" begin
        ...
        f = RadialFlow()
        @test typeof(f) == FlowMessagePassingRules.RadialFlowPlaceholder

        # check when creating placeholder
        f = FlowMessagePassingRules.RadialFlowPlaceholder()
        @test typeof(f) == FlowMessagePassingRules.RadialFlowPlaceholder

        # check when creating empty planar flow
        f = FlowMessagePassingRules.RadialFlowEmpty(3)
        @test typeof(f) == FlowMessagePassingRules.RadialFlowEmpty{3}
        @test getdim(f) == 3
    ...
    @testset "Prepare-Compile" begin
        f = FlowMessagePassingRules.RadialFlowPlaceholder()
        @test FlowMessagePassingRules.prepare(2, f) == FlowMessagePassingRules.RadialFlowEmpty(2)
        @test FlowMessagePassingRules.prepare(5, f) == FlowMessagePassingRules.RadialFlowEmpty(5)
        @test FlowMessagePassingRules.prepare(9, f) == FlowMessagePassingRules.RadialFlowEmpty(9)

        f = FlowMessagePassingRules.RadialFlowEmpty(1)
        params = [1.0, 2.0, 3.0]
        ...
        f = FlowMessagePassingRules.RadialFlowEmpty(2)
    ...
    @testset "nr_params" begin
        for k in 1:10
            f = FlowMessagePassingRules.RadialFlowEmpty(k)
            fc = compile(rng, f)
