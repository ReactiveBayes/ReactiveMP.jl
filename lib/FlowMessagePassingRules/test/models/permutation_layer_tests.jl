...
@testitem "models:Permutation Layer" tags = [:models] begin
    ...
    @testset "Constructor" begin
        ...
        @test typeof(layer) == FlowMessagePassingRules.PermutationLayerPlaceholder

        # check for layer with available P matrix
        P = PermutationMatrix(rng, 5)
        layer = PermutationLayer(5, P)
        @test layer.P == P
        @test typeof(layer) <: PermutationLayer
        @test typeof(layer) <: FlowMessagePassingRules.AbstractLayer
        @test typeof(layer.P) <: PermutationMatrix

        # check for layer with unknown P matrix
        layer = PermutationLayer(rng, 4)
    ...
    @testset "Prepare-Compile" begin
        layer = FlowMessagePassingRules._prepare(rng, 3, FlowMessagePassingRules.PermutationLayerPlaceholder())
        @test typeof(layer) <: Tuple
        @test typeof(layer[1]) <: PermutationLayer
        @test layer[1].dim == 3
        @test size(layer[1].P) == (3, 3)

        P = PermutationMatrix(rng, 4)
    ...
    @testset "Jacobian" begin
        ...
        P = PermutationMatrix(rng, 3)
        layer = PermutationLayer(3, P)
        @test jacobian(layer, randn(rng, 3)) == P
        @test jacobian(layer, randn(rng, 3)) == P
        @test jacobian.(layer, [randn(rng, 3), randn(rng, 3)]) == [P, P]

        # check invjacobian function
        P = PermutationMatrix(rng, 3)
        layer = PermutationLayer(3, P)
        @test inv_jacobian(layer, randn(rng, 3)) == P'
        @test inv_jacobian(layer, randn(rng, 3)) == P'
        @test inv_jacobian.(layer, [randn(rng, 3), randn(rng, 3)]) == [P', P']
    end
    ...
    @testset "Utility Jacobian" begin
        ...
        P = PermutationMatrix(rng, 3)
        layer = PermutationLayer(3, P)
        @test det_jacobian(layer, randn(rng, 3)) == det(P)
        @test det_jacobian(layer) == det(P)
        @test absdet_jacobian(layer, randn(rng, 3)) == 1.0
        @test absdet_jacobian(layer) == 1.0
        @test logdet_jacobian(layer, randn(rng, 3)) == 0.0
        @test logdet_jacobian(layer) == 0.0
        @test logabsdet_jacobian(layer, randn(rng, 3)) == 0.0
