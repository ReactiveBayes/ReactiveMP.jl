...
@testitem "quality:closure" tags = [:quality] begin
    ...
    self = only(filter(((_, info),) -> info.name == "FlowMessagePassingRules", deps))
    closure = Set{String}()
...
@testitem "quality:rules" tags = [:quality] begin
    ...
    @test isempty(check_rules(FlowMessagePassingRules))
    @test isempty(check_rule_ambiguities(FlowMessagePassingRules))
end
