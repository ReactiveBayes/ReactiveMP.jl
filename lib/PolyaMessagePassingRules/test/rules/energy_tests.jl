...
@testitem "rules:BinomialPolya:energy" tags = [:rules] setup = [PolyaReference] begin
    ...
    for algorithm in (BinomialPolyaApproximation(), BinomialPolyaApproximation(samples = 100))
        energy = getresult(call_average_energy(BinomialPolya; q, algorithm))
...
@testitem "rules:MultinomialPolya:energy" tags = [:rules] setup = [PolyaReference] begin
    ...
    for (N, π) in ((1, [0.2, 0.3, 0.5]), (3, [0.2, 0.3, 0.5]), (5, [0.6, 0.1, 0.3]))
        energy = getresult(call_average_energy(MultinomialPolya; q = (x = Multinomial(N, π), N = PointMass(N), ψ = q_ψ)))
        ...
        @test getresult(call_average_energy(MultinomialPolya; q = (x = PointMass(x), N = PointMass(N), ψ = q_ψ))) ≈ expected rtol = 1.0e-6
    end
    # Observed counts and a point-mass ψ.
    @test getresult(call_average_energy(MultinomialPolya; q = (x = PointMass([30, 70]), N = PointMass(100), ψ = PointMass([0.5])))) ≈ 23.76 atol = 0.1
    # The energy of a discrete x is never negative.
    energy = getresult(call_average_energy(MultinomialPolya; q = (x = Multinomial(100, [0.2, 0.3, 0.5]), N = PointMass(100), ψ = MvNormalMeanCovariance([0.1, -0.2], [2.0 0.5; 0.5 1.5]))))
    @test energy > 0
end
