...
@testitem "rules:GCV:gaussian extension, messages towards μ" tags = [:rules] setup = [GCVRulesTestUtils] begin
    ...
    for elq in elqs
        ...
        for (node, inputs) in (
            ...
            for input in inputs
                ...
                result = getresult(call_message_update_rule(node, :μ; m = (out = elq, m...), q))
                reference = getresult(call_message_update_rule(node, :μ; m = (out = normal, m...), q))
    ...
    @test all(mean_var(getresult(call_message_update_rule(NormalMeanVariance, :μ; m = (out = elq, v = PointMass(2.0))))) .≈ (elq_mean, elq_var + 2.0))
    @test all(mean_var(getresult(call_message_update_rule(NormalMeanPrecision, :μ; m = (out = elq,), q = (τ = GammaShapeRate(3.0, 2.0),)))) .≈ (elq_mean, elq_var + 2 / 3))
end
...
@testitem "marginalrules:GCV:gaussian extension, the joint of out and μ" tags = [:rules] setup = [GCVRulesTestUtils] begin
    ...
    for (node, target, param) in ((NormalMeanVariance, (:out, :μ, :v), :v), (NormalMeanPrecision, (:out, :μ, :τ), :τ))
        ...
        result = getresult(call_marginal_update_rule(node, target; m = (out = elq, μ = m_μ, m_param...)))
        reference = getresult(call_marginal_update_rule(node, target; m = (out = normal, μ = m_μ, m_param...)))
    ...
    for (node, q) in ((NormalMeanVariance, (v = InverseGamma(3.0, 4.0),)), (NormalMeanPrecision, (τ = GammaShapeRate(3.0, 2.0),)))
        result = getresult(call_marginal_update_rule(node, (:out, :μ); m = (out = elq, μ = m_μ), q))
        reference = getresult(call_marginal_update_rule(node, (:out, :μ); m = (out = normal, μ = m_μ), q))
    ...
    joint = getresult(call_marginal_update_rule(NormalMeanVariance, (:out, :μ); m = (out = elq, μ = m_μ), q = (v = InverseGamma(3.0, 4.0),)))
