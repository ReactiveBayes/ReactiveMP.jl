...
@testitem "rules:AND" tags = [:rules] begin
    ...
    @test_message_update_rule(node = AND, target = :out, cases = [(m = (in1 = Bernoulli(0.3), in2 = Bernoulli(0.5)),) => ExpectedWithLogScale(Bernoulli(0.15), 0)])
    @test_message_update_rule(node = AND, target = :in1, cases = [(m = (out = Bernoulli(0.3), in2 = Bernoulli(0.4)),) => ExpectedWithLogScale(Bernoulli(0.54 / 1.24), log(1.24))])
    @test_message_update_rule(node = AND, target = :in2, cases = [(m = (out = Bernoulli(0.6), in1 = Bernoulli(0.5)),) => ExpectedWithLogScale(Bernoulli(0.5 / 0.9), log(0.9))])
...
@testitem "rules:OR" tags = [:rules] begin
    ...
    @test_message_update_rule(node = OR, target = :in1, cases = [(m = (out = Bernoulli(0.3), in2 = Bernoulli(0.4)),) => ExpectedWithLogScale(Bernoulli(0.3 / 0.84), log(0.84))])
    @test_message_update_rule(node = OR, target = :in2, cases = [(m = (out = Bernoulli(0.6), in1 = Bernoulli(0.5)),) => ExpectedWithLogScale(Bernoulli(0.6 / 1.1), log(1.1))])
    @test_marginal_update_rule(
...
@testitem "rules:NOT" tags = [:rules] begin
    ...
    @test_message_update_rule(node = NOT, target = :out, cases = [(m = (in = Bernoulli(0.3),),) => ExpectedWithLogScale(Bernoulli(0.7), 0)])
...
@testitem "rules:IMPLY" tags = [:rules] begin
    ...
    @test_message_update_rule(node = IMPLY, target = :in1, cases = [(m = (out = Bernoulli(0.2), in2 = Bernoulli(0.5)),) => ExpectedWithLogScale(Bernoulli(0.5 / 0.7), log(0.7))])
    @test_message_update_rule(node = IMPLY, target = :in2, cases = [(m = (out = Bernoulli(0.3), in1 = Bernoulli(0.4)),) => ExpectedWithLogScale(Bernoulli(0.3 / 0.76), log(0.76))])
    @test_marginal_update_rule(
