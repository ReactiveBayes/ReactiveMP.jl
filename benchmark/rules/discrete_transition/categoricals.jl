...
using ReactiveMP
import ReactiveMP:
    Marginal, Contingency, Categorical, Marginalisation, normalize!
...
function add_discrete_transition_categoricals_rule_benchmarks(SUITE)
    ...
    for n_categories in 2:5
        for categorical_size in [3, 5, 10, 20]
            discrete_transition_mean_field_categoricals_rule(
                SUITE["Categoricals"], n_categories, categorical_size
            )
            discrete_transition_bp_categoricals_rule(
                SUITE["Categoricals"], n_categories, categorical_size
            )
            if n_categories > 4
                discrete_transition_structured_categoricals_rule(
                    SUITE["Categoricals"], n_categories, categorical_size
                )
            end
        end
...
function discrete_transition_structured_categoricals_rule(
    ...
    incoming_messages = ntuple(
        ...
        n_messages,
    ) # One less message since we're computing :out
    ...
    SUITE["Structured"]["$(n_categories) categories, $(cat_size) size"] = @benchmarkable ReactiveMP.rule(
        ...
        ReactiveMP.call_rule_make_node(
            DiscreteTransition, DiscreteTransition, nothing
        ),
    )
end

function discrete_transition_mean_field_categoricals_rule(
    SUITE, n_categories, cat_size
)
    incoming_marginals = ntuple(
        i -> if i == 1
