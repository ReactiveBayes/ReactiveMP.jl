...
function add_discrete_transition_a_rule_benchmarks(SUITE)
    ...
    for n_categories in 2:5
        for categorical_size in [3, 5, 10, 20]
            discrete_transition_mean_field_a_rule(
                SUITE["a"], n_categories, categorical_size
            )
            discrete_transition_bp_a_rule(
                SUITE["a"], n_categories, categorical_size
            )
        end
    end
...
function discrete_transition_bp_a_rule(SUITE, n_categories, cat_size)
    incoming_marginal = (
        ReactiveMP.Marginal(
            Contingency(rand(ntuple(_ -> cat_size, n_categories)...)),
            false,
            false,
            nothing
        ),
