# unknown inverse
@rule DeltaFn((:in, k), Marginalisation) (q_ins::JointNormal, m_in::NormalDistributionsFamily, meta::DeltaMeta{M, I}) where {M <: GenUnscented, I <: Nothing} = begin
...
@rule DeltaFn((:in, _), Marginalisation) (m_out::NormalDistributionsFamily, m_ins::Nothing, meta::DeltaMeta{M, I}) where {M <: GenUnscented, I <: Function} = begin
    return approximate(AsDistribution(NormalDistributionsFamily), ReactiveMP.getmethod(meta), getnodefn(meta, Val(:in)), (m_out,))
end
...
@rule DeltaFn((:in, k), Marginalisation) (
    ...
    return approximate(AsDistribution(NormalDistributionsFamily), ReactiveMP.getmethod(meta), getnodefn(meta, Val(:in), k), (m_out, m_ins...))
end