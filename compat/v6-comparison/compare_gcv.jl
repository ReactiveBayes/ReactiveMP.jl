...
@testset "GCVMessagePassingRules against v6" begin
    for p in PARAMETERS
        ...
        for (target, other) in ((:y, :x), (:x, :y))
            ...
            v7 = getresult(call_message_update_rule(GCV, target; m, q = rest))
            ...
            v7 = getresult(call_message_update_rule(GCV, target; q = merge(m, rest)))
        ...
        for target in (:z, :κ, :ω)
            ...
            v7 = getresult(call_message_update_rule(GCV, target; q = others, clusters = ((:y, :x) => joint,)))
            ...
            v7 = getresult(call_message_update_rule(GCV, target; q = merge((y = p.y, x = p.x), others)))
        ...
        v7 = getresult(call_marginal_update_rule(GCV, (:y, :x); m = (y = p.y, x = p.x), q = rest))
        ...
        if !(p.z isa PointMass)
            v7 = getresult(call_average_energy(GCV; clusters = ((:y, :x) => joint,), q = rest))
            ...
            v7 = getresult(call_average_energy(GCV; q = merge((y = p.y, x = p.x), rest)))
    ...
    elq = getresult(call_message_update_rule(GCV, :z; q = (y = NormalMeanVariance(3.0, 1.0), x = NormalMeanVariance(1.0, 2.0), κ = NormalMeanVariance(0.8, 0.4), ω = NormalMeanVariance(1.2, 0.5))))
    ...
    for (node, v6_node, parameter, point, marginal) in ((NormalMeanVariance, ReactiveMP.NormalMeanVariance, :v, PointMass(2.0), GammaInverse(3.0, 2.0)), (NormalMeanPrecision, ReactiveMP.NormalMeanPrecision, :τ, PointMass(0.5), GammaShapeRate(3.0, 2.0)))
        for (m, q) in ((NamedTuple{(:out, parameter)}((elq, point)), NamedTuple()), ((out = elq,), NamedTuple{(parameter,)}((marginal,))))
            v7 = getresult(call_message_update_rule(node, :μ; m, q))
        ...
        v7 = getresult(call_marginal_update_rule(node, (:out, :μ, parameter); m = NamedTuple{(:out, :μ, parameter)}((elq, μ, point))))
        ...
        v7 = getresult(call_marginal_update_rule(node, (:out, :μ); m = (out = elq, μ = μ), q = NamedTuple{(parameter,)}((marginal,))))
