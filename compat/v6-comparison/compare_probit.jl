...
@testset "ProbitMessagePassingRules against v6" begin
    for algorithm in (ProbitEP(), DefaultAlgorithm()), m_in in (CAVITIES..., PointMass(1.0), PointMass(-0.5))
        v7 = getresult(call_message_update_rule(Probit, :out; m = (in = m_in,), algorithm))
    ...
    for m_out in OUTPUTS, m_in in CAVITIES
        v7 = getresult(call_message_update_rule(Probit, :in; m = (out = m_out, in = m_in)))
    ...
    for m_out in OUTPUTS
        v7 = getresult(call_message_update_rule(Probit, :in; m = (out = m_out,), algorithm = DefaultAlgorithm()))
    ...
    for p in (1.0, 0.0), m_in in CAVITIES
        v7 = getresult(call_marginal_update_rule(Probit, (:out, :in); m = (out = PointMass(p), in = m_in)))
    ...
    for q_out in OUTPUTS, q_in in CAVITIES, p in (32, 100)
        v7 = getresult(call_average_energy(Probit; q = (out = q_out, in = q_in), algorithm = ProbitEP(p)))
