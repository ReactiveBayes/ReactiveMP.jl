...
# ARMA(10, 4) model on the stock data. RxInfer v7 branch on ReactiveMP v7. Ported per the v6 → v7
# guide: AR comes from its own package, `ARMeta` is `ARVMP` and `@meta` is `@algorithm`.
...
ar_unit(order) = AutoregressiveMessagePassingRules.ar_unit(Multivariate, order)
...
arma_result = infer(
    ...
    algorithm = ar_algorithm(q_order),
    returnvars = KeepLast(),
    iterations = 20,
