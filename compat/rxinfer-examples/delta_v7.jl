...
# RxInfer v7 branch on ReactiveMP v7. Ported per the v6 → v7 guide: `meta`/`@meta` become
# `algorithm`/`@algorithm`, and Linearization runs under `DeltaApproximation(method = …)`.
using RxInfer
...
min_algorithm = @algorithm begin
    smooth_min() -> DeltaApproximation(method = Linearization())
end
