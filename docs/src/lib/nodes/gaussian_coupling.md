# [GaussianCoupling node](@id lib-nodes-gaussian-coupling)

The `GaussianCoupling` node encodes the stochastic pairwise Gaussian coupling (a bilinear interaction)
...
## [Interfaces](@id lib-nodes-gaussian-coupling-interfaces)
...
## [Factorization](@id lib-nodes-gaussian-coupling-factorization)
...
When `a` is supplied as a constant — the intended usage, as in `x[j] ~ GaussianCoupling(x[i], -A[i, j])` — this factorization is applied automatically, because constant interfaces are always factorized out of the local cluster. `MeanField()` is **not** supported: there are no `q(out)q(in)` message rules and no mean-field average energy.

## [Improperness](@id lib-nodes-gaussian-coupling-improperness)
...
- The joint marginal `q(out, in)` is proper only when the incoming precisions dominate the coupling, ``w_{\mathrm{out}} w_{\mathrm{in}} > a^2``. This is a *local* condition on a single factor; it is **not** the condition under which the method as a whole works — see [Accuracy](@ref lib-nodes-gaussian-coupling-accuracy) for that.
...
## [Accuracy](@id lib-nodes-gaussian-coupling-accuracy)
