...
- A test suite for `lib/MessagePassingRulesBase`, run with `make test-base` (same `test_args` syntax as `make test`) and in CI by the new `LibTests.yml` on Julia 1.10, 1.11 and 1.12. It carries Aqua with every check on, and `quality:closure`, which asserts that `ExponentialFamily` is absent both from the package's resolved dependency graph and from what a fresh process loads.
- `compat/v6-comparison/README.md`, recording why the environment's manifest is pinned to Julia 1.10 (a manifest resolved on 1.13 fails to load on the floor), how to regenerate and verify it, and the BayesBase 1.x constraint it depends on.
