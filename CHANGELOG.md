...
### Changed
- The Phase 4.5 design brief is recorded in `PHASES.md`, with the reasoning in `DISCUSSION.md` §3.19. Decided: RxInfer is refactored for the new engine as its own major release, so v7 does not keep RxInfer 5.x's surface. Rocket stays the engine's substrate. Deferred messages materialise exactly as in v6, because this is load-bearing for the correctness of reactive message passing. The slice's roughly 45 rules are ported into `StandardMessagePassingRules` and `MessagePassingRulesApproximations`. The base package gains a cluster over a whole group, which Delta's joint marginal needs. Ten further proposals await sign-off. Design documents only; no code has changed.
