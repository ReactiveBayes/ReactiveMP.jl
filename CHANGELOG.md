...
- The planned rule/node macro surface becomes fully keyword-based, with the rule body as an ordinary Julia lambda over a real arguments object rather than a body the macro rewrites. Symbols are used throughout (`towards = :out`, `m[:μ]`, `interfaces = [:out, ...]`), since a real lambda body makes them load-bearing rather than decorative. `@allocate` and `@logscale` are deleted rather than renamed, becoming the `preallocate` keyword and an ordinary `annotate!` call. Design only; no macros exist yet. See `PLAN.md` § Rule surface and `DISCUSSION.md` §3.14.
...
- `MessagePassingRulesBase`: a cluster over a whole group. A cluster is the tuple of its members, `q[(:y, :x)]`, and `q[:y, :x]` is shorthand for it. Inside a cluster a group's name means all of its members jointly: `q[(:in,)]` is the joint over the group `in`, which Delta's `q_ins` needs, and `q[:out, :in]` mixes a single interface with a group. The same spelling works in rule `args`, in dependencies, and as a marginal rule's `towards = (:in,)`. At run time `args.q[(:in,)]` is inferred and allocation-free. A one-member cluster of a single interface is rejected in favour of `q[:μ]`.
...
- `MessagePassingRulesBase`: the rule definition macros `@define_message_update_rule`, `@define_marginal_update_rule` and `@define_average_energy`. Everything is a keyword and the body is an ordinary lambda over the slots it names, in the canonical order `(output, algo, ctx, args, ann)`. `args` declares messages `m[:μ]::T`, marginals `q[:μ]::T`, clusters `q[:y, :x]::T` and whole groups `m[:inputs...]::T`, and generates the dispatch signature in canonical key order. `towards = (:m, k)` binds `k` in both the body and `preallocate`. The macros also take `inplace`/`preallocate`, a per-rule `pure` override and the `ctx = (...)` service list. The spec records its source text, file and line. A rule that omits `algorithm` runs under its node's default, and must then be loaded after the node.
