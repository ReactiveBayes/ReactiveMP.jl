...
- `lib/` holds the new packages, each with its own suite and the same `test_args` syntax:
  ...
  six nodes, the univariate distributions, the logic nodes, the multivariate normals, the matrix and Wishart nodes and the arithmetic nodes so far), `MessagePassingRulesApproximations` (`make test-approximations`;
