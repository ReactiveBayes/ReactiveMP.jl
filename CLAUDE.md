...
Tests are `@testitem` blocks (164 of them across 24 files), each self-contained and
...
**Every test item carries a tag.** The taxonomy is `:nodes` (29) and `:engine` (133 —
...
review's findings resolved; Phase 6 (the node packages) is closed: every node is in its own package and `legacy/` is gone. Phase 7, completing the engine, is under way: its entry brief is written and answered (log scales fixed, RxInfer adapted on a local branch, diagnostics as activation options; `DISCUSSION.md` §3.46).
