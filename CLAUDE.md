...
for s in check compare_standard compare_approximations compare_delta compare_gaussian_coupling; do julia --project=compat/v6-comparison compat/v6-comparison/$s.jl; done
...
Tests are `@testitem` blocks (135 of them across 22 files), each self-contained and
...
**Every test item carries a tag.** The taxonomy is `:nodes` (23) and `:engine` (110 —
...
- `lib/` holds the new packages, each with its own suite and the same `test_args` syntax:
  ...
  `GaussianCouplingMessagePassingRules` (`make test-gaussian-coupling`). Siblings are wired with `[deps]` and `[sources]`. No Manifest under `lib/` is committed; the local ones are gitignored.
...
review's findings resolved; Phase 6 (the node packages) is under way: steps 1 and 2 are done, and step 3, the small nodes, is next.
