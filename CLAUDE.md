...
Tests are `@testitem` blocks (129 of them across 21 files), each self-contained and
...
**Every test item carries a tag.** The taxonomy is `:nodes` (21) and `:engine` (107 —
...
- `lib/` holds the new packages, each with its own suite and the same `test_args` syntax:
  ...
  six nodes, the univariate distributions, the logic nodes, the multivariate normals, the matrix and Wishart nodes, the arithmetic nodes and GammaMixture so far), `MessagePassingRulesApproximations` (`make test-approximations`;
...
The current work is the rule/node rewrite; Phase 4.5 is closed, and Phase 5 is in progress (steps 1–7
done). From
