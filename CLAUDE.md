...
- `lib/` holds the new packages, each with its own suite and the same `test_args` syntax:
  ...
  six nodes, the univariate distributions, the logic nodes, the multivariate normals, the matrix and Wishart nodes, `+`, `-` and `dot` so far), `MessagePassingRulesApproximations` (`make test-approximations`;
...
The current work is the rule/node rewrite; Phase 4.5 is closed, and Phase 5 is in progress (steps 1–6
done). From
